...
inline std::size_t for_each_line(
    std::istream& stream, Function function,
...
template <typename Function>
inline std::size_t for_each_line_n(
    std::istream& stream, const std::size_t& n, Function function,
    const std::size_t& buffer_size = one_kilobyte) {
  std::string buffer;
  buffer.reserve(buffer_size);
  std::size_t line_count = 0;
...
template <typename Function>
inline std::size_t for_each_line(
    const std::string& file_name, Function function,
    const std::size_t& buffer_size = one_kilobyte) {
...
template <typename Function>
inline std::size_t for_each_line_n(
    const std::string& file_name, const std::size_t& n, Function function,
    const std::size_t& buffer_size = one_kilobyte) {
  std::ifstream stream(file_name.c_str());
  if (stream)
...
template <typename Function>
inline std::size_t for_each_line_conditional(
    std::istream& stream, Function function,
    const std::size_t& buffer_size = one_kilobyte) {
  std::string buffer;
  buffer.reserve(buffer_size);
...
template <typename Function>
inline std::size_t for_each_line_n_conditional(
    std::istream& stream, const std::size_t& n, Function function,
    const std::size_t& buffer_size = one_kilobyte) {
  std::string buffer;
  buffer.reserve(buffer_size);
...
struct is_valid_iterator {
  typedef
      typename details::enable_if<details::supported_iterator_type<T>::value,
                                  T>::type type;
};
...
#define register_stl_container1(C)      \
  ...
  struct is_stl_container<C<T1, T2> > { \
    typedef yes_t result_t;             \
  };

#define register_stl_container2(C)                 \
  template <typename T1, typename T2, typename T3> \
  struct is_stl_container<C<T1, T2, T3> > {        \
    typedef yes_t result_t;                        \
  };

register_stl_container1(std::vector) register_stl_container1(std::deque)
    register_stl_container1(std::list) register_stl_container1(std::queue)
        register_stl_container1(std::stack) register_stl_container2(std::set)
...
inline std::string::value_type* to_ptr(std::string& s) { return &s[0]; }
...
inline bool string_to_type_converter(const std::string& s, T& t) {
  return string_to_type_converter<const char*, T>(to_ptr(s),
                                                  to_ptr(s) + s.size(), t);
}
...
#define strtk_string_to_type_begin(Type)                   \
  ...
  inline bool string_to_type_converter_impl(               \
      const Iterator& begin, const Iterator& end, Type& t, \
...
inline std::size_t load_from_text_file(
    std::istream& stream, Sequence<T, Allocator>& sequence,
    const std::size_t& buffer_size = one_kilobyte) {
  if (!stream) return 0;
...
inline std::size_t load_from_text_file(
    std::istream& stream, std::set<T, Comparator, Allocator>& set,
    const std::size_t& buffer_size = one_kilobyte) {
  if (!stream) return 0;
...
template <typename T, typename Comparator, typename Allocator>
inline std::size_t load_from_text_file(
    std::istream& stream, std::multiset<T, Comparator, Allocator>& multiset,
    const std::size_t& buffer_size = one_kilobyte) {
  if (!stream) return 0;
...
inline std::size_t load_from_text_file(
    std::istream& stream, std::queue<T, Container>& queue,
    const std::size_t& buffer_size = one_kilobyte) {
  if (!stream) return 0;
...
template <typename T, typename Container>
inline std::size_t load_from_text_file(
    std::istream& stream, std::stack<T, Container>& stack,
    const std::size_t& buffer_size = one_kilobyte) {
  if (!stream) return 0;
...
template <typename T, typename Allocator,
          template <typename, typename> class Sequence>
inline std::size_t load_from_text_file(
    const std::string& file_name, Sequence<T, Allocator>& sequence,
    const std::size_t& buffer_size = one_kilobyte) {
  std::ifstream stream(file_name.c_str());
  if (!stream)
    return 0;
...
template <typename T, typename Comparator, typename Allocator>
inline std::size_t load_from_text_file(
    const std::string& file_name, std::set<T, Comparator, Allocator>& set,
    const std::size_t& buffer_size = one_kilobyte) {
  std::ifstream stream(file_name.c_str());
  if (!stream)
    return 0;
...
template <typename T, typename Container>
inline std::size_t load_from_text_file(
    const std::string& file_name, std::queue<T, Container>& queue,
    const std::size_t& buffer_size = one_kilobyte) {
  std::ifstream stream(file_name.c_str());
  if (!stream)
    return 0;
...
template <typename T, typename Container>
inline std::size_t load_from_text_file(
    const std::string& file_name, std::stack<T, Container>& stack,
    const std::size_t& buffer_size = one_kilobyte) {
  std::ifstream stream(file_name.c_str());
  if (!stream)
    return 0;
...
template <typename T, typename Comparator, typename Allocator>
inline std::size_t write_to_text_file(
    std::ostream& stream, const std::set<T, Comparator, Allocator>& set,
    const std::string& delimiter = "") {
  if (!stream) return 0;
...
template <typename T, typename Comparator, typename Allocator>
inline std::size_t write_to_text_file(
    const std::string& file_name, const std::set<T, Comparator, Allocator>& set,
    const std::string& delimiter = "") {
  std::ofstream stream(file_name.c_str());
  if (!stream)
    return 0;
...
class adapter {
 public:
  ...
  adapter(T* const begin, T* const end) : begin_(begin), end_(end) {}

  adapter(const std::pair<T*, T*>& r) : begin_(r.first), end_(r.second) {}
  ...
  inline std::size_t size() const { return std::distance(begin_, end_); }

  inline operator std::string() const { return stringify(begin_, end_); }

  inline const T& operator[](const std::size_t& index) const {
    return *(begin_ + index);
  }

  inline T& operator[](const std::size_t& index) { return *(begin_ + index); }
...
struct single_delimiter_predicate {
 public:
  ...
  inline bool operator()(const T& d) const { return delimiter_ == d; }
...
template <typename Predicate, typename T, typename Allocator,
          template <typename, typename> class Sequence>
inline std::size_t remove_inplace(Predicate predicate,
...
inline void remove_inplace(const std::string::value_type c, std::string& s) {
  const std::size_t removal_count = remove_inplace(
      ...
      const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size()));
  if (removal_count > 0) {
...
inline void remove_inplace(Predicate predicate, std::string& s) {
  const std::size_t removal_count =
      remove_inplace(predicate, const_cast<char*>(to_ptr(s)),
...
inline void remove_consecutives_inplace(const char* rem_chars, std::string& s) {
  ...
  const std::size_t removal_count = remove_consecutives_inplace(
      multiple_char_delimiter_predicate(
          rem_chars, rem_chars + details::strnlength(rem_chars, 256)),
      const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size()));
  if (removal_count > 0) {
    s.resize(s.size() - removal_count);
  }
...
inline void remove_consecutives_inplace(Predicate predicate, std::string& s) {
  if (s.empty()) return;
  const std::size_t removal_count =
      remove_consecutives_inplace(predicate, const_cast<char*>(to_ptr(s)),
                                  const_cast<char*>(to_ptr(s) + s.size()));
  if (removal_count > 0) {
    s.resize(s.size() - removal_count);
  }
...
inline void remove_consecutives_inplace(Sequence<T, Allocator>& sequence) {
  const std::size_t removal_count =
      remove_consecutives_inplace(sequence.begin(), sequence.end());
  sequence.resize(sequence.size() - removal_count);
}
...
inline void remove_trailing(Predicate predicate, std::string& s) {
  if (s.empty()) return;
  const std::size_t removal_count =
      remove_trailing(predicate, const_cast<char*>(to_ptr(s)),
                      const_cast<char*>(to_ptr(s) + s.size()));
  if (removal_count > 0) {
    s.resize(s.size() - removal_count);
  }
...
inline void remove_leading(Predicate predicate, std::string& s) {
  if (s.empty()) return;
  const std::size_t removal_count =
      remove_leading(predicate, const_cast<char*>(to_ptr(s)),
                     const_cast<char*>(to_ptr(s) + s.size()));
  if (removal_count > 0) {
    s.resize(s.size() - removal_count);
  }
...
void remove_empty_strings(Sequence<std::string, Allocator>& seq) {
  struct is_empty {
    static inline bool check(const std::string& s) { return s.empty(); }
  };
...
inline void replace(const std::string::value_type& c0,
  ...
  replace(c0, c1, const_cast<char*>(to_ptr(s)),
          const_cast<char*>(to_ptr(s) + s.size()));
}
...
inline std::size_t replace_pattern(const InputIterator s_begin,
                                   ...
                                   const InputIterator p_begin,
                                   const InputIterator p_end,  // Pattern
                                   const InputIterator r_begin,
                                   const InputIterator r_end,  // Replacement
                                   OutputIterator out) {
...
inline void sort(std::string& s) { std::sort(s.begin(), s.end()); }
...
inline bool match_impl(
    const Iterator pattern_begin, const Iterator pattern_end,
  ...
  while ((p_itr != pattern_end) && (d_itr != data_end)) {
    if (match_zero_or_more == *p_itr) {
      while ((p_itr != pattern_end) &&
             (*p_itr == match_zero_or_more || *p_itr == match_zero_or_one)) {
        ++p_itr;
      }

      if (p_itr == pattern_end) return true;
      ...
      ++d_itr;
    } else if ((*p_itr == match_zero_or_one) ||
               Comparator::cmp(*p_itr, *d_itr)) {
      ++d_itr;
      ++p_itr;
    } else
      return false;
...
inline bool match(const Iterator pattern_begin, const Iterator pattern_end,
  ...
  return details::match_impl<details::cs_match>(
      pattern_begin, pattern_end, data_begin, data_end, match_zero_or_more,
      match_zero_or_one);
}
...
inline bool match(const std::string& wild_card, const std::string& str) {
  ...
  */
  return details::match_impl<details::cs_match>(
      to_ptr(wild_card), to_ptr(wild_card) + wild_card.size(), to_ptr(str),
      to_ptr(str) + str.size(), '*', '?');
}
...
inline bool imatch(const InputIterator begin1, const InputIterator end1,
  ...
  while (end1 != itr1) {
    // if (std::toupper(*itr1, std::locale::classic()) != std::toupper(*it2,
    // std::locale::classic()))
...
inline bool imatch(const std::string& s1, const std::string& s2) {
  return imatch(to_ptr(s1), to_ptr(s1) + s1.size(), to_ptr(s2),
                to_ptr(s2) + s2.size());
}
...
inline std::size_t find_all(const Iterator pattern_begin,
                            const Iterator pattern_end, const Iterator begin,
...
template <typename Iterator, typename Range, typename Allocator,
          template <typename, typename> class Sequence>
inline std::size_t find_all(const Iterator pattern_begin,
                            const Iterator pattern_end, const Iterator begin,
                            const Iterator end,
...
inline std::size_t ifind(const std::string& pattern, const std::string& data) {
  if (pattern.size() > data.size()) return std::string::npos;

  const char* result_itr =
      std::search(to_ptr(data), to_ptr(data) + data.size(), to_ptr(pattern),
                  to_ptr(pattern) + pattern.size(), imatch_char);
...
inline std::size_t find_all(const std::string& pattern, const std::string& data,
  ...
  return find_all(to_ptr(pattern), to_ptr(pattern) + pattern.size(),
                  to_ptr(data), to_ptr(data) + data.size(), out);
}
...
class tokenizer {
 private:
  ...
  class tokenizer_iterator
   ...
   public:
    explicit inline tokenizer_iterator(
        const iterator begin, const iterator end, const Predicate& predicate,
        ...
        : predicate_(predicate),
          ...
          compress_delimiters_(
              tokenize_options::perform_compress_delimiters(tokenize_option)),
          include_1st_delimiter_(
              tokenize_options::perform_include_1st_delimiter(tokenize_option)),
          include_all_delimiters_(
              tokenize_options::perform_include_all_delimiters(
                  tokenize_option)),
          include_delimiters_(include_1st_delimiter_ ||
                              include_all_delimiters_),
    ...
    inline T operator*() const { return current_token_; }
 ...
 public:
  ...
  inline const_iterator_ref begin() const { return begin_itr_; }

  inline const_iterator_ref end() const { return end_itr_; }
...
class range_to_type_back_inserter_iterator
 ...
 public:
  ...
  range_to_type_back_inserter_iterator(
      const range_to_type_back_inserter_iterator& itr)
  ...
  inline range_to_type_back_inserter_iterator& operator=(
    ...
    if (string_to_type_converter(r.first, r.second, t)) sequence_.push_back(t);
  ...
  inline range_to_type_back_inserter_iterator& operator*() { return (*this); }
...
class range_to_type_inserter_iterator
 ...
 public:
  ...
  inline range_to_type_inserter_iterator& operator=(
      const std::pair<Iterator, Iterator>& r) {
    value_type t;
    if (string_to_type_converter(r.first, r.second, t)) set_.insert(t);
  ...
  inline range_to_type_inserter_iterator& operator*() { return (*this); }

  inline range_to_type_inserter_iterator& operator++() { return (*this); }

  inline range_to_type_inserter_iterator operator++(int) { return (*this); }
...
class range_to_type_push_inserter_iterator
 ...
 public:
  ...
  range_to_type_push_inserter_iterator(
      const range_to_type_push_inserter_iterator& itr)
  ...
  inline range_to_type_push_inserter_iterator& operator=(
    ...
    if (string_to_type_converter(r.first, r.second, t)) container_.push(t);
  ...
  inline range_to_type_push_inserter_iterator& operator*() { return (*this); }
...
class inserter_with_valuetype_iterator
    : public std::iterator<std::output_iterator_tag, typename Set::value_type,
                           void, void, void> {
 public:
  explicit inline inserter_with_valuetype_iterator(Set& set) : set_(set) {}
  ...
  inline void operator()(const typename Set::value_type& v) { set_.insert(v); }

  inline inserter_with_valuetype_iterator& operator*() { return (*this); }

  inline inserter_with_valuetype_iterator& operator++() { return (*this); }

  inline inserter_with_valuetype_iterator operator++(int) { return (*this); }
...
class push_inserter_iterator
 ...
 public:
  ...
  inline push_inserter_iterator<Container>& operator*() { return (*this); }

  inline push_inserter_iterator<Container>& operator++() { return (*this); }

  inline push_inserter_iterator<Container> operator++(int) { return (*this); }
...
class range_to_ptr_type_iterator
 ...
 public:
  ...
  inline range_to_ptr_type_iterator& operator=(
      const range_to_ptr_type_iterator& itr) {
    if (this != &itr) {
  ...
  inline range_to_ptr_type_iterator& operator*() { return (*this); }

  inline range_to_ptr_type_iterator& operator++() { return (*this); }

  inline range_to_ptr_type_iterator operator++(int) { return (*this); }
...
template <typename T>
inline range_to_ptr_type_iterator<T> range_to_ptr_type(
    T* pointer, std::size_t& insert_count) {
...
class counting_back_inserter_iterator
 ...
 public:
  ...
  inline void operator()(const T&) { ++counter_; }
...
class functional_inserter_iterator
 ...
 public:
  ...
  inline functional_inserter_iterator& operator=(
      const functional_inserter_iterator& itr) {
    if (this != &itr) {
  ...
  inline functional_inserter_iterator& operator*() { return (*this); }

  inline functional_inserter_iterator& operator++() { return (*this); }

  inline functional_inserter_iterator operator++(int) { return (*this); }
...
namespace details {
template <typename DelimiterPredicate, typename Iterator,
...
inline std::size_t split_compress_delimiters(
  ...
  while (end != range.second) {
    if (delimiter(*range.second)) {
      ...
      if (end != range.second) ++range.second;
      ++token_count;
    } else
      ++range.second;
  }
...
inline std::size_t split(
  ...
  const bool compress_delimiters =
      split_options::perform_compress_delimiters(split_option);
  const bool include_1st_delimiter =
      split_options::perform_include_1st_delimiter(split_option);
  const bool include_all_delimiters =
      (!include_1st_delimiter) &&
      split_options::perform_include_all_delimiters(split_option);
  const bool include_delimiters =
      include_1st_delimiter || include_all_delimiters;
...
template <typename DelimiterPredicate, typename Iterator,
          typename OutputIterator>
inline std::size_t split(
    const char* delimiters, const std::pair<Iterator, Iterator>& range,
    OutputIterator out,
    const split_options::type split_option = split_options::default_mode) {
  if (1 == details::strnlength(delimiters, 256))
    return split(
        single_delimiter_predicate<std::string::value_type>(delimiters[0]),
        range.first, range.second, out, split_option);
  else
...
template <typename DelimiterPredicate, typename Iterator,
          typename OutputIterator>
inline std::size_t split(
    const std::string& delimiters, const std::pair<Iterator, Iterator>& range,
    OutputIterator out,
    const split_options::type split_option = split_options::default_mode) {
...
template <typename OutputIterator>
inline std::size_t split(
    const char* delimiters, const std::string& str, OutputIterator out,
    const split_options::type& split_option = split_options::default_mode) {
  if (1 == details::strnlength(delimiters, 256))
    return split(
        single_delimiter_predicate<std::string::value_type>(delimiters[0]),
        to_ptr(str), to_ptr(str) + str.size(), out, split_option);
  else
    return split(multiple_char_delimiter_predicate(delimiters), to_ptr(str),
                 to_ptr(str) + str.size(), out, split_option);
}

template <typename OutputIterator>
inline std::size_t split(
    const std::string& delimiters, const std::string& str, OutputIterator out,
    const split_options::type& split_option = split_options::default_mode) {
  if (1 == delimiters.size())
    return split(
        single_delimiter_predicate<std::string::value_type>(delimiters[0]),
...
template <typename OutputIterator>
inline std::size_t split(
    const std::string::value_type delimiter, const std::string& str,
    OutputIterator out,
    const split_options::type& split_option = split_options::default_mode) {
...
template <typename Allocator, template <typename, typename> class Sequence>
inline std::size_t split(
    const char* delimiters, const std::string& str,
    Sequence<std::pair<const char*, const char*>, Allocator>& sequence,
    const split_options::type& split_option = split_options::default_mode) {
  if (1 == details::strnlength(delimiters, 256))
    return split(
        single_delimiter_predicate<std::string::value_type>(delimiters[0]),
        to_ptr(str), to_ptr(str) + str.size(), std::back_inserter(sequence),
        split_option);
  else
    return split(multiple_char_delimiter_predicate(delimiters), to_ptr(str),
                 to_ptr(str) + str.size(), std::back_inserter(sequence),
                 split_option);
}

template <typename Allocator, template <typename, typename> class Sequence>
inline std::size_t split(
    const std::string& delimiters, const std::string& str,
    Sequence<std::pair<const char*, const char*>, Allocator>& sequence,
    const split_options::type& split_option = split_options::default_mode) {
  if (1 == delimiters.size())
    return split(
...
inline std::size_t split(
    const DelimiterPredicate& delimiter, const std::string& str,
    OutputIterator out,
    const split_options::type& split_option = split_options::default_mode) {
  return split(delimiter, to_ptr(str), to_ptr(str) + str.size(), out,
               split_option);
}

template <typename DelimiterPredicate, typename Iterator,
          typename OutputIterator>
inline std::size_t split_n(
    const DelimiterPredicate& delimiter, const Iterator begin,
    const Iterator end, const std::size_t& token_count, OutputIterator out,
    const split_options::type& split_option = split_options::default_mode) {
  ...
  while (end != range.second) {
    if (delimiter(*range.second)) {
      if (include_delimiters) {
        ...
        if (++match_count >= token_count) return match_count;
...
inline std::size_t split_n(
    ...
    const split_options::type& split_option = split_options::default_mode) {
  return split_n(multiple_char_delimiter_predicate(delimiters), to_ptr(str),
                 to_ptr(str) + str.size(), token_count, out, split_option);
}
...
inline std::size_t split_n(
  ...
  if (1 == delimiters.size())
    return split_n(
        single_delimiter_predicate<std::string::value_type>(delimiters[0]),
        to_ptr(str), to_ptr(str) + str.size(), token_count, out, split_option);
  else
    return split_n(multiple_char_delimiter_predicate(delimiters), to_ptr(str),
                   to_ptr(str) + str.size(), token_count, out, split_option);
}
...
template <typename InputIterator, typename OutputIterator>
inline std::size_t split_n(
    const std::string& delimiters, const InputIterator begin,
    const InputIterator end, const std::size_t& token_count, OutputIterator out,
    const split_options::type& split_option = split_options::default_mode) {
  typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
  details::convert_type_assert<itr_type>();
  if (1 == delimiters.size())
    return split_n(
        single_delimiter_predicate<std::string::value_type>(delimiters[0]),
        begin, end, token_count, out, split_option);
  else
    return split_n(multiple_char_delimiter_predicate(delimiters), begin, end,
...
inline std::size_t split_n(
  ...
  return split_n(single_delimiter_predicate<std::string::value_type>(delimiter),
                 to_ptr(str), to_ptr(str) + str.size(), token_count, out,
                 split_option);
}
...
inline std::size_t split_n(
    ...
    const std::size_t& token_count, OutputIterator out,
    const split_options::type& split_option = split_options::default_mode) {
  return split_n(delimiter, to_ptr(str), to_ptr(str) + str.size(), token_count,
                 out, split_option);
}
...
static const std::string uri_expression(
    "((https?|ftp)\\://"
    "((\\[?(\\d{1,3}\\.){3}\\d{1,3}\\]?)|(([-a-zA-Z0-9]+\\.)+[a-zA-Z]{2,4}))("
    "\\:\\d+)?(/[-a-zA-Z0-9._?,+&amp;%$#=~\\\\]+)*/?)");
static const std::string email_expression(
    "([\\w\\-\\.]+)@((\\[([0-9]{1,3}\\.){3}[0-9]{1,3}\\])|(([\\w\\-]+\\.)+)([a-"
    "zA-Z]{2,4}))");
static const std::string ip_expression(
    "(([0-2]*[0-9]+[0-9]+)\\.([0-2]*[0-9]+[0-9]+)\\.([0-2]*[0-9]+[0-9]+)\\.([0-"
    "2]*[0-9]+[0-9]+))");
...
template <typename InputIterator, typename OutputIterator>
inline std::size_t split_regex(
    const boost::regex& delimiter_expression, const InputIterator begin,
    const InputIterator end, OutputIterator out,
...
template <typename InputIterator, typename OutputIterator>
inline std::size_t split_regex(
    const std::string& delimiter_expression, const InputIterator begin,
    const InputIterator end, OutputIterator out,
    const regex_match_mode::type mode = regex_match_mode::match_all) {
...
template <typename OutputIterator>
inline std::size_t split_regex(
    const std::string& delimiter_expression, const std::string& text,
    OutputIterator out,
    const regex_match_mode::type mode = regex_match_mode::match_all) {
  return split_regex(delimiter_expression, text.begin(), text.end(), out, mode);
}

template <typename OutputIterator>
inline std::size_t split_regex(
    const boost::regex& delimiter_expression, const std::string& text,
    OutputIterator out,
    const regex_match_mode::type mode = regex_match_mode::match_all) {
  return split_regex(delimiter_expression, text.begin(), text.end(), out, mode);
}
...
template <typename InputIterator, typename OutputIterator>
inline std::size_t split_regex_n(
    const boost::regex& delimiter_expression, const InputIterator begin,
    const InputIterator end, const std::size_t& token_count, OutputIterator out,
    const regex_match_mode::type mode = regex_match_mode::match_all) {
...
class offset_predicate {
 public:
  ...
  inline bool operator!() const { return (0 == offset_list_size); }

  inline void reset() const { current_index_ = 0; }

  inline std::size_t size() const { return offset_list_size; }
...
inline offset_predicate<12> offsets(const int& v1, const int& v2, const int& v3,
  ...
  const int offset_list[12] = {v1, v2, v3, v4,  v5,  v6,
...
inline bool split_pair(const InputIterator begin, const InputIterator end,
                       const Predicate& delimiter, OutputPair& v1,
                       OutputPair& v2) {
...
inline bool split_pair(const std::string::value_type delimiter,
  ...
  return split_pair(
      ...
      single_delimiter_predicate<std::string::value_type>(delimiter), v1, v2);
}
...
inline bool split_pair(const DelimiterPredicate& delimiter,
                       ...
                       std::pair<const char*, const char*>& v1,
                       std::pair<const char*, const char*>& v2) {
  return split_pair(to_ptr(str), to_ptr(str) + str.size(), delimiter, v1, v2);
}
...
inline const char* first_non_repeated_char(const char* begin, const char* end) {
  ...
  for (std::size_t i = 0; i < lut_size; ++i) {
    if (lut[i] < position) position = lut[i];
  }
...
inline std::size_t first_non_repeated_char(const std::string& str) {
  if (str.empty()) return static_cast<std::size_t>(std::string::npos);

  const char* itr =
      first_non_repeated_char(to_ptr(str), to_ptr(str) + str.size());
...
inline void convert_bin_to_hex(const unsigned char* begin,
  ...
  static const unsigned short hex_lut[] = {
      ...
      0x4231, 0x4331, 0x4431, 0x4531, 0x4631, 0x3032, 0x3132, 0x3232, 0x3332,
      0x3432, 0x3532, 0x3632, 0x3732, 0x3832, 0x3932, 0x4132, 0x4232, 0x4332,
      0x4432, 0x4532, 0x4632, 0x3033, 0x3133, 0x3233, 0x3333, 0x3433, 0x3533,
      0x3633, 0x3733, 0x3833, 0x3933, 0x4133, 0x4233, 0x4333, 0x4433, 0x4533,
      0x4633, 0x3034, 0x3134, 0x3234, 0x3334, 0x3434, 0x3534, 0x3634, 0x3734,
      0x3834, 0x3934, 0x4134, 0x4234, 0x4334, 0x4434, 0x4534, 0x4634, 0x3035,
      0x3135, 0x3235, 0x3335, 0x3435, 0x3535, 0x3635, 0x3735, 0x3835, 0x3935,
      0x4135, 0x4235, 0x4335, 0x4435, 0x4535, 0x4635, 0x3036, 0x3136, 0x3236,
      0x3336, 0x3436, 0x3536, 0x3636, 0x3736, 0x3836, 0x3936, 0x4136, 0x4236,
      0x4336, 0x4436, 0x4536, 0x4636, 0x3037, 0x3137, 0x3237, 0x3337, 0x3437,
      0x3537, 0x3637, 0x3737, 0x3837, 0x3937, 0x4137, 0x4237, 0x4337, 0x4437,
      0x4537, 0x4637, 0x3038, 0x3138, 0x3238, 0x3338, 0x3438, 0x3538, 0x3638,
      0x3738, 0x3838, 0x3938, 0x4138, 0x4238, 0x4338, 0x4438, 0x4538, 0x4638,
      0x3039, 0x3139, 0x3239, 0x3339, 0x3439, 0x3539, 0x3639, 0x3739, 0x3839,
      0x3939, 0x4139, 0x4239, 0x4339, 0x4439, 0x4539, 0x4639, 0x3041, 0x3141,
      0x3241, 0x3341, 0x3441, 0x3541, 0x3641, 0x3741, 0x3841, 0x3941, 0x4141,
      0x4241, 0x4341, 0x4441, 0x4541, 0x4641, 0x3042, 0x3142, 0x3242, 0x3342,
      0x3442, 0x3542, 0x3642, 0x3742, 0x3842, 0x3942, 0x4142, 0x4242, 0x4342,
      0x4442, 0x4542, 0x4642, 0x3043, 0x3143, 0x3243, 0x3343, 0x3443, 0x3543,
      0x3643, 0x3743, 0x3843, 0x3943, 0x4143, 0x4243, 0x4343, 0x4443, 0x4543,
      0x4643, 0x3044, 0x3144, 0x3244, 0x3344, 0x3444, 0x3544, 0x3644, 0x3744,
      0x3844, 0x3944, 0x4144, 0x4244, 0x4344, 0x4444, 0x4544, 0x4644, 0x3045,
...
inline void convert_bin_to_hex(
    const std::pair<unsigned char*, unsigned char*>& r, unsigned char* out) {
...
inline bool convert_hex_to_bin(const std::string& hex_data,
                               std::string& output) {
  if (hex_data.empty() || (1 == (hex_data.size() % 2))) return false;
...
inline void convert_bin_to_base64(const std::string& binary_data,
  ...
  std::size_t resize = convert_bin_to_base64(
      to_ptr(binary_data), to_ptr(binary_data) + binary_data.size(),
      const_cast<char*>(to_ptr(output)));
...
inline void convert_base64_to_bin(const std::string& binary_data,
  ...
  std::size_t resize = convert_base64_to_bin(
      to_ptr(binary_data), to_ptr(binary_data) + binary_data.size(),
      const_cast<char*>(to_ptr(output)));
  output.resize(resize);
}
...
inline void convert_to_printable_chars(std::string& str) {
  convert_to_printable_chars(
      reinterpret_cast<unsigned char*>(const_cast<char*>(to_ptr(str))),
...
inline bool twoway_bitwise_interleave(const unsigned char* begin1,
  ...
  static const unsigned short interleave_table[interleave_table_size] = {
      ...
      0x0010, 0x0011, 0x0014, 0x0015,  // 0x00 - 0x07
      0x0040, 0x0041, 0x0044, 0x0045,
      0x0050, 0x0051, 0x0054, 0x0055,  // 0x08 - 0x0F
      0x0100, 0x0101, 0x0104, 0x0105,
      0x0110, 0x0111, 0x0114, 0x0115,  // 0x10 - 0x17
      0x0140, 0x0141, 0x0144, 0x0145,
      0x0150, 0x0151, 0x0154, 0x0155,  // 0x18 - 0x1F
      0x0400, 0x0401, 0x0404, 0x0405,
      0x0410, 0x0411, 0x0414, 0x0415,  // 0x20 - 0x27
      0x0440, 0x0441, 0x0444, 0x0445,
      0x0450, 0x0451, 0x0454, 0x0455,  // 0x28 - 0x2F
      0x0500, 0x0501, 0x0504, 0x0505,
      0x0510, 0x0511, 0x0514, 0x0515,  // 0x30 - 0x37
      0x0540, 0x0541, 0x0544, 0x0545,
      0x0550, 0x0551, 0x0554, 0x0555,  // 0x38 - 0x3F
      0x1000, 0x1001, 0x1004, 0x1005,
      0x1010, 0x1011, 0x1014, 0x1015,  // 0x40 - 0x47
      0x1040, 0x1041, 0x1044, 0x1045,
      0x1050, 0x1051, 0x1054, 0x1055,  // 0x48 - 0x4F
      0x1100, 0x1101, 0x1104, 0x1105,
      0x1110, 0x1111, 0x1114, 0x1115,  // 0x50 - 0x57
      0x1140, 0x1141, 0x1144, 0x1145,
      0x1150, 0x1151, 0x1154, 0x1155,  // 0x58 - 0x5F
      0x1400, 0x1401, 0x1404, 0x1405,
      0x1410, 0x1411, 0x1414, 0x1415,  // 0x60 - 0x67
      0x1440, 0x1441, 0x1444, 0x1445,
      0x1450, 0x1451, 0x1454, 0x1455,  // 0x68 - 0x6F
      0x1500, 0x1501, 0x1504, 0x1505,
      0x1510, 0x1511, 0x1514, 0x1515,  // 0x70 - 0x77
      0x1540, 0x1541, 0x1544, 0x1545,
      0x1550, 0x1551, 0x1554, 0x1555,  // 0x78 - 0x7F
      0x4000, 0x4001, 0x4004, 0x4005,
      0x4010, 0x4011, 0x4014, 0x4015,  // 0x80 - 0x87
      0x4040, 0x4041, 0x4044, 0x4045,
      0x4050, 0x4051, 0x4054, 0x4055,  // 0x88 - 0x8F
      0x4100, 0x4101, 0x4104, 0x4105,
      0x4110, 0x4111, 0x4114, 0x4115,  // 0x90 - 0x97
      0x4140, 0x4141, 0x4144, 0x4145,
      0x4150, 0x4151, 0x4154, 0x4155,  // 0x98 - 0x9F
      0x4400, 0x4401, 0x4404, 0x4405,
      0x4410, 0x4411, 0x4414, 0x4415,  // 0xA0 - 0xA7
      0x4440, 0x4441, 0x4444, 0x4445,
      0x4450, 0x4451, 0x4454, 0x4455,  // 0xA8 - 0xAF
      0x4500, 0x4501, 0x4504, 0x4505,
      0x4510, 0x4511, 0x4514, 0x4515,  // 0xB0 - 0xB7
      0x4540, 0x4541, 0x4544, 0x4545,
      0x4550, 0x4551, 0x4554, 0x4555,  // 0xB8 - 0xBF
      0x5000, 0x5001, 0x5004, 0x5005,
      0x5010, 0x5011, 0x5014, 0x5015,  // 0xC0 - 0xC7
      0x5040, 0x5041, 0x5044, 0x5045,
      0x5050, 0x5051, 0x5054, 0x5055,  // 0xC8 - 0xCF
      0x5100, 0x5101, 0x5104, 0x5105,
      0x5110, 0x5111, 0x5114, 0x5115,  // 0xD0 - 0xD7
      0x5140, 0x5141, 0x5144, 0x5145,
      0x5150, 0x5151, 0x5154, 0x5155,  // 0xD8 - 0xDF
      0x5400, 0x5401, 0x5404, 0x5405,
      0x5410, 0x5411, 0x5414, 0x5415,  // 0xE0 - 0xE7
      0x5440, 0x5441, 0x5444, 0x5445,
      0x5450, 0x5451, 0x5454, 0x5455,  // 0xE8 - 0xEF
      0x5500, 0x5501, 0x5504, 0x5505,
      0x5510, 0x5511, 0x5514, 0x5515,  // 0xF0 - 0xF7
      0x5540, 0x5541, 0x5544, 0x5545,
      0x5550, 0x5551, 0x5554, 0x5555  // 0xF8 - 0xFF
  };
...
template <>
struct interleave_ary<sizeof(unsigned short)> {
  typedef unsigned short type;
};
template <>
struct interleave_ary<sizeof(unsigned int)> {
...
inline void bitwise_transform(const bitwise_operation::type& operation,
  ...
  out.resize(str1.size());
  bitwise_transform(operation, to_ptr(str1), to_ptr(str1) + str1.size(),
                    to_ptr(str2), const_cast<char*>(to_ptr(out)));
}
...
class token_grid {
 ...
 private:
  struct store {
    ...
    inline bool remove_row(const std::size_t& row) {
      ...
      token_list_t::iterator remove_end =
          token_list.begin() + r.first + number_of_tokens;
    ...
    inline bool remove_row_range(const std::size_t& r0, const std::size_t& r1) {
      ...
      token_list_t::iterator remove_end =
          token_list.begin() + rr0.first + number_of_tokens;
    ...
    struct remove_column_impl {
      ...
      inline void update(store& idx) {
        ...
        while (current_row < idx.row_index.size()) {
          ...
          if (number_of_tokens > column) break;
      ...
      inline void process(store& idx) {
        ...
        for (std::size_t i = 0; i < idx.row_index.size(); ++i) {
          ...
          if (token_count > idx.max_column) idx.max_column = token_count;
        }
    ...
    inline static range_t null_range() {
      static const range_t null_range_ =
  ...
  struct row_processor {
    ...
    inline void operator()(const range_t& range) {
      if (0 == std::distance(range.first, range.second)) return;

      row_start_index_ = static_cast<index_t>(idx_.token_list.size());

      std::size_t token_count =
          split(token_predicate_, range.first, range.second,
                std::back_inserter(idx_.token_list), split_mode_);
      ...
      if (token_count > idx_.max_column) idx_.max_column = token_count;
    }
 ...
 public:
  ...
  class row_type {
   ...
   public:
    row_type() : index_(std::numeric_limits<std::size_t>::max()), size_(0) {}

    row_type(const std::size_t& index, const store& dsv_index)
        : index_(index),
          size_(dsv_index.token_count(index)),
          begin_(dsv_index.token_list.begin() +
    ...
    inline void as_string(std::string& out) const { out = as_string(); }

    template <typename T0, typename T1, typename T2, typename T3, typename T4,
              typename T5, typename T6, typename T7, typename T8, typename T9>
    inline bool parse_with_index(
        const std::size_t& col0, const std::size_t& col1,
        ...
        const std::size_t& col8, const std::size_t& col9, T0& t0, T1& t1,
        T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7, T8& t8, T9& t9) const {
    ...
    inline bool parse_with_index(
        ...
        const std::size_t& col4, const std::size_t& col5,
        const std::size_t& col6, const std::size_t& col7,
        const std::size_t& col8, T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
    ...
    inline bool parse_with_index(
        ...
        const std::size_t& col2, const std::size_t& col3,
        const std::size_t& col4, const std::size_t& col5,
        const std::size_t& col6, const std::size_t& col7, T0& t0, T1& t1,
        T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7) const {
      if (!process(*(begin_ + col0), t0)) return false;
      if (!process(*(begin_ + col1), t1)) return false;
      if (!process(*(begin_ + col2), t2)) return false;
      if (!process(*(begin_ + col3), t3)) return false;
    ...
    inline bool parse_with_index(const std::size_t& col0,
                                 ...
                                 const std::size_t& col6, T0& t0, T1& t1,
    ...
    inline bool parse_with_index(const std::size_t& col0,
                                 ...
                                 const std::size_t& col3,
                                 const std::size_t& col4,
                                 const std::size_t& col5, T0& t0, T1& t1,
    ...
    inline bool parse_with_index(const std::size_t& col0,
                                 ...
                                 const std::size_t& col2,
                                 const std::size_t& col3,
                                 const std::size_t& col4, T0& t0, T1& t1,
                                 T2& t2, T3& t3, T4& t4) const {
      if (!process(*(begin_ + col0), t0)) return false;
      if (!process(*(begin_ + col1), t1)) return false;
      if (!process(*(begin_ + col2), t2)) return false;
      if (!process(*(begin_ + col3), t3)) return false;
    ...
    inline bool parse_with_index(const std::size_t& col0,
                                 const std::size_t& col1,
                                 const std::size_t& col2,
                                 const std::size_t& col3, T0& t0, T1& t1,
                                 T2& t2, T3& t3) const {
      if (!process(*(begin_ + col0), t0)) return false;
      if (!process(*(begin_ + col1), t1)) return false;
      if (!process(*(begin_ + col2), t2)) return false;
    ...
    inline bool parse_with_index(const std::size_t& col0,
                                 const std::size_t& col1,
                                 const std::size_t& col2, T0& t0, T1& t1,
                                 T2& t2) const {
      if (!process(*(begin_ + col0), t0)) return false;
      if (!process(*(begin_ + col1), t1)) return false;
      if (!process(*(begin_ + col2), t2)) return false;
    ...
    template <typename T0, typename T1, typename T2, typename T3, typename T4,
              typename T5, typename T6, typename T7, typename T8, typename T9>
    inline bool parse(T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6,
    ...
    template <typename T0, typename T1, typename T2, typename T3, typename T4,
              typename T5>
    inline bool parse(T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5) const {
      if (!process(*(begin_ + 0), t0)) return false;
      if (!process(*(begin_ + 1), t1)) return false;
      if (!process(*(begin_ + 2), t2)) return false;
      if (!process(*(begin_ + 3), t3)) return false;
    ...
    inline bool parse(const col_range_t& range,
      ...
      if (!validate_column_range(range)) return false;
  ...
  token_grid(const std::string& input_buffer,
      ...
      : file_name_(""),
        buffer_(reinterpret_cast<unsigned char*>(
            const_cast<char*>(to_ptr(input_buffer)))),
        buffer_size_(input_buffer_size),
        min_column_count_(0),
        max_column_count_(0),
        options_(options),
  ...
  inline std::size_t row_count() const { return dsv_index_.row_index.size(); }
    ...
    for (std::size_t i = row_range.first; i < row_range.second; ++i) {
      ...
      if (index < dsv_index_.token_count(row)) {
        ...
        process_token_checked(
            *(dsv_index_.token_list.begin() + (row.first + index)), out);
      }
    }
  ...
  template <typename T, typename Comparator, typename Allocator>
  inline void extract_column_checked(
      const std::size_t& index, std::set<T, Comparator, Allocator>& set) const {
  ...
  inline bool extract_column(const row_range_t& row_range,
                             ...
                             const std::size_t& index1, OutputIterator0 out0,
  ...
  inline bool extract_column(const row_range_t& row_range,
                             ...
                             const std::size_t& index3, OutputIterator0 out0,
  ...
  inline bool extract_column(const row_range_t& row_range,
                             ...
                             OutputIterator1 out1, OutputIterator2 out2,
                             OutputIterator3 out3, OutputIterator4 out4) const {
    if ((index0 > max_column_count_) || (index1 > max_column_count_) ||
        (index2 > max_column_count_) || (index3 > max_column_count_) ||
    ...
    std::size_t max_index = std::max(
        index4, std::max(std::max(index0, index1), std::max(index2, index3)));
  ...
  inline bool remove_row_if(const row_range_t& row_range, Predicate predicate) {
    if (row_range_invalid(row_range)) return false;
  ...
  inline std::size_t remove_token_if(const row_range_t& row_range,
                                     Predicate predicate) {
    if (row_range_invalid(row_range)) return 0;
  ...
  inline void enforce_column_count(const row_range_t& row_range,
    ...
    if (row_range_invalid(row_range)) return;
  ...
  inline void enforce_min_max_column_count(
      const row_range_t& row_range, const std::size_t& min_column_count,
  ...
  inline void clear(const bool force_delete_buffer = false) {
    if (load_from_file_ || force_delete_buffer) delete[] buffer_;
  ...
  inline std::size_t column_width(const std::size_t& col,
    ...
    for (std::size_t i = row_range.first; i < row_range.second; ++i) {
      ...
      if (col < dsv_index_.token_count(r)) {
        ...
        result = std::max<std::size_t>(std::distance(range.first, range.second),
                                       result);
      }
    }
  ...
  inline bool join_row(const std::size_t& row, const std::string& delimiter,
                       std::string& result) {
    if (row >= dsv_index_.row_index.size()) return false;

    const row_index_range_t& r = dsv_index_.row_index[row];

    token_list_t::const_iterator itr = dsv_index_.token_list.begin() + r.first;
    token_list_t::const_iterator end =
        dsv_index_.token_list.begin() + r.second +
        (row < (dsv_index_.row_index.size() - 1) ? 1 : 0);

    result.reserve(delimiter.size() * dsv_index_.token_count(r) +
                   std::distance(itr->first, end->second));

    bool appended = false;

    while (end != itr) {
      if (!delimiter.empty() && appended) result.append(delimiter);
  ...
  inline bool join_row(const std::size_t& row, Predicate predicate,
    ...
    token_list_t::const_iterator itr =
        (dsv_index_.token_list.begin() + r.first);
    token_list_t::const_iterator end =
        dsv_index_.token_list.begin() + r.second +
        (row < (dsv_index_.row_index.size() - 1) ? 1 : 0);
  ...
  inline bool join_column(const std::size_t& col, const row_range_t& row_range,
                          const std::string& delimiter,
  ...
  inline bool join_column(const std::size_t& col, const std::string& delimiter,
                          std::string& result) const {
  ...
  template <typename Predicate>
  inline bool join_column(const std::size_t& col, const row_range_t& row_range,
                          Predicate predicate, const std::string& delimiter,
                          std::string& result) const {
    if (col > max_column_count_)
      return false;
    else if (row_range_invalid(row_range))
  ...
  static inline token_grid::options default_options() { return options(); }
 ...
 private:
  ...
  struct insufficient_number_of_columns {
    insufficient_number_of_columns(const std::size_t& noc) : num_of_cols(noc) {}
  ...
  class double_quotes_predicate {
   public:
    ...
    inline void reset() { in_bracket_range_ = false; }
  ...
  inline bool load() {
    if (load_from_file_ && !load_buffer_from_file()) return false;
    ...
    if (!options_.support_dquotes) {
      ...
      strtk::split(
          text_newline_predicate, buffer_, buffer_ + buffer_size_,
      ...
      strtk::split(
          text_newline_predicate, buffer_, buffer_ + buffer_size_,
          strtk::functional_inserter(row_processor<double_quotes_predicate>(
  ...
  inline void process_token(const range_t& range, OutputIterator out) const {
    typedef
        typename std::iterator_traits<OutputIterator>::value_type output_type;
...
struct filter_non_empty_range {
 public:
  filter_non_empty_range(OutputIterator out) : out_(out) {}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  const std::size_t parsed_token_count =
      split_n(delimiters, begin, end, token_count, token_list,
              split_options::compress_delimiters);
...
#define strtk_cmpstmt(N)                                            \
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) strtk_cmpstmt(10) strtk_cmpstmt(11) strtk_cmpstmt(12)
              strtk_cmpstmt(13) strtk_cmpstmt(14)
                  strtk_cmpstmt(15) return string_to_type_converter(
                      (*itr).first, (*itr).second, t16);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) strtk_cmpstmt(10) strtk_cmpstmt(11) strtk_cmpstmt(12)
              strtk_cmpstmt(13)
                  strtk_cmpstmt(14) return string_to_type_converter(
                      (*itr).first, (*itr).second, t15);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) strtk_cmpstmt(10) strtk_cmpstmt(11) strtk_cmpstmt(12)
              strtk_cmpstmt(13) return string_to_type_converter(
                  (*itr).first, (*itr).second, t14);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) strtk_cmpstmt(10) strtk_cmpstmt(11)
              strtk_cmpstmt(12) return string_to_type_converter(
                  (*itr).first, (*itr).second, t13);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) strtk_cmpstmt(10)
              strtk_cmpstmt(11) return string_to_type_converter(
                  (*itr).first, (*itr).second, t12);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) strtk_cmpstmt(10) return string_to_type_converter(
              (*itr).first, (*itr).second, t11);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) return string_to_type_converter((*itr).first,
                                                           (*itr).second, t10);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(
          8) return string_to_type_converter((*itr).first, (*itr).second, t9);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(
          7) return string_to_type_converter((*itr).first, (*itr).second, t8);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) return string_to_type_converter(
          (*itr).first, (*itr).second, t7);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) return string_to_type_converter((*itr).first,
                                                       (*itr).second, t6);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(
      4) return string_to_type_converter((*itr).first, (*itr).second, t5);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(
      3) return string_to_type_converter((*itr).first, (*itr).second, t4);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list;
  strtk_cmpstmt(1) strtk_cmpstmt(2) return string_to_type_converter(
      (*itr).first, (*itr).second, t3);
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list;
  strtk_cmpstmt(1) return string_to_type_converter((*itr).first, (*itr).second,
                                                   t2);
#undef strtk_cmpstmt
}
...
inline std::size_t parse_impl(const InputIterator begin,
                              const InputIterator end,
                              const std::string& delimiters, OutputIterator out,
  ...
  if (1 == delimiters.size())
    return split(
        single_delimiter_predicate<std::string::value_type>(delimiters[0]),
        begin, end, out, split_option);
  else
    return split(multiple_char_delimiter_predicate(delimiters), begin, end, out,
                 split_option);
}
}  // namespace details

#define strtk_def_parse_cont(Type)                                             \
  template <typename InputIterator, typename T, typename Allocator>            \
  inline std::size_t parse(const InputIterator begin, const InputIterator end, \
                           const std::string& delimiters,                      \
                           Type<T, Allocator>& sequence,                       \
                           const split_options::type& split_option =           \
                               split_options::compress_delimiters) {           \
    return details::parse_impl(begin, end, delimiters,                         \
                               range_to_type_back_inserter(sequence),          \
                               split_option);                                  \
  }

strtk_def_parse_cont(std::vector) strtk_def_parse_cont(std::deque)
...
#undef strtk_def_parse_cont
        ...
        template <typename InputIterator, typename T, typename Comparator,
                  typename Allocator>
        inline std::size_t
    parse(const InputIterator begin, const InputIterator end,
          const std::string& delimiters,
...
inline std::size_t parse(const std::pair<InputIterator, InputIterator>& range,
                         ...
                         const split_options::type& split_option =
                             split_options::compress_delimiters) {
  return details::parse_impl(range.first, range.second, delimiters,
                             range_to_type_back_inserter(sequence),
                             split_option);
}
 ...
 private:
  ...
  class sequence_adder_impl : public container_adder_base {
   public:
    ...
    sequence_adder_impl(sequence_t& seq) : sequence_(seq) {}
  ...
  class set_adder_impl : public container_adder_base {
   public:
    set_adder_impl(Set<T, Comparator, Allocator>& set) : set_(set) {}
 ...
 public:
  ...
  container_adder(std::vector<T, Allocator>& vec)
      : container_adder_base_(
            new (buffer) sequence_adder_impl<T, Allocator, std::vector>(vec)) {}

  template <typename T, typename Allocator>
  container_adder(std::deque<T, Allocator>& deq)
      : container_adder_base_(
            new (buffer) sequence_adder_impl<T, Allocator, std::deque>(deq)) {}

  template <typename T, typename Allocator>
  container_adder(std::list<T, Allocator>& list)
      : container_adder_base_(
            new (buffer) sequence_adder_impl<T, Allocator, std::list>(list)) {}

  template <typename T, typename Comparator, typename Allocator>
  container_adder(std::set<T, Comparator, Allocator>& set)
      : container_adder_base_(new (
            buffer) set_adder_impl<T, Comparator, Allocator, std::set>(set)) {}

  template <typename T, typename Comparator, typename Allocator>
  container_adder(std::multiset<T, Comparator, Allocator>& multiset)
      : container_adder_base_(
            new (buffer)
                set_adder_impl<T, Comparator, Allocator, std::multiset>(
                    multiset)) {}

  template <typename T, typename Container, typename Comparator>
  container_adder(std::priority_queue<T, Container, Comparator>& pq)
      : container_adder_base_(new (buffer)
                                  pq_adder_impl<T, Container, Comparator>(pq)) {
  }

  template <typename T, typename Container>
  container_adder(std::queue<T, Container>& queue)
      : container_adder_base_(new (
            buffer) stack_queue_adder_impl<T, Container, std::queue>(queue)) {}

  template <typename T, typename Container>
  container_adder(std::stack<T, Container>& stack)
      : container_adder_base_(new (
            buffer) stack_queue_adder_impl<T, Container, std::stack>(stack)) {}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  if (1 == delimiters.size())
    split(single_delimiter_predicate<std::string::value_type>(delimiters[0]),
          begin, end, std::back_inserter(token_list),
          split_options::compress_delimiters);
  else
    split(multiple_char_delimiter_predicate(delimiters), begin, end,
          std::back_inserter(token_list), split_options::compress_delimiters);
...
#define strtk_cmpstmt(N)                                            \
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) strtk_cmpstmt(10)
              strtk_cmpstmt(11) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) strtk_cmpstmt(10) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
                  const std::string& delimiters, T1& t1, T2& t2, T3& t3, T4& t4,
                  T5& t5, T6& t6, T7& t7, T8& t8, details::container_adder ca) {
  typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
  typedef std::pair<InputIterator, InputIterator> iterator_type;
  typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
  ...
  iterator_type_ptr itr = token_list.begin();
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7)
          strtk_cmpstmt(8) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
                  const std::string& delimiters, T1& t1, T2& t2, T3& t3, T4& t4,
                  T5& t5, T6& t6, T7& t7, details::container_adder ca) {
  typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
  typedef std::pair<InputIterator, InputIterator> iterator_type;
  typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
  ...
  iterator_type_ptr itr = token_list.begin();
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6)
          strtk_cmpstmt(7) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
                  const std::string& delimiters, T1& t1, T2& t2, T3& t3, T4& t4,
                  T5& t5, T6& t6, details::container_adder ca) {
  typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
  typedef std::pair<InputIterator, InputIterator> iterator_type;
  typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
  ...
  iterator_type_ptr itr = token_list.begin();
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
                  const std::string& delimiters, T1& t1, T2& t2, T3& t3, T4& t4,
                  T5& t5, details::container_adder ca) {
  typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
  typedef std::pair<InputIterator, InputIterator> iterator_type;
  typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
  ...
  iterator_type_ptr itr = token_list.begin();
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list.begin();
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3)
      strtk_cmpstmt(4) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list.begin();
  strtk_cmpstmt(1) strtk_cmpstmt(2)
      strtk_cmpstmt(3) return ca(itr, token_list.end());
}
...
inline bool parse(const InputIterator begin, const InputIterator end,
  ...
  iterator_type_ptr itr = token_list.begin();
  strtk_cmpstmt(1) strtk_cmpstmt(2) return ca(itr, token_list.end());
}

template <typename InputIterator, typename T1>
inline bool parse(const InputIterator begin, const InputIterator end,
                  const std::string& delimiters, T1& t1,
                  details::container_adder ca) {
  typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
  typedef std::pair<InputIterator, InputIterator> iterator_type;
  typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
  ...
  iterator_type_ptr itr = token_list.begin();
  strtk_cmpstmt(1) return ca(itr, token_list.end());
#undef strtk_cmpstmt
}

#define strtk_def_parse_n_cont(Type)                                           \
  template <typename InputIterator, typename T, typename Allocator>            \
  inline std::size_t parse_n(                                                  \
      const InputIterator begin, const InputIterator end,                      \
    ...
    if (1 == delimiters.size())                                                \
      split_n(                                                                 \
          single_delimiter_predicate<std::string::value_type>(delimiters[0]),  \
          begin, end, n, range_to_type_back_inserter(sequence), split_option); \
    else                                                                       \
...
strtk_def_parse_n_cont(std::vector) strtk_def_parse_n_cont(std::deque)
...
#undef strtk_def_parse_n_cont
        ...
        template <typename InputIterator, typename T, typename Comparator,
                  typename Allocator>
        inline std::size_t
    parse_n(const InputIterator begin, const InputIterator end,
            const std::string& delimiters, const std::size_t& n,
            std::set<T, Comparator, Allocator>& set,
            const split_options::type& split_option =
                split_options::compress_delimiters) {
  ...
  if (1 == delimiters.size())
    split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]),
            begin, end, n, range_to_type_inserter(set), split_option);
  else
    split_n(multiple_char_delimiter_predicate(delimiters), begin, end, n,
            range_to_type_inserter(set), split_option);
...
inline std::size_t parse_n(const InputIterator begin, const InputIterator end,
  ...
  if (1 == delimiters.size())
    split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]),
            begin, end, n, range_to_type_inserter(multiset), split_option);
  else
    split_n(multiple_char_delimiter_predicate(delimiters), begin, end, n,
            range_to_type_inserter(multiset), split_option);
...
inline std::size_t parse_n(const InputIterator begin, const InputIterator end,
  ...
  if (1 == delimiters.size())
    split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]),
            begin, end, n, range_to_type_push_inserter(queue), split_option);
  else
    split_n(multiple_char_delimiter_predicate(delimiters), begin, end, n,
            range_to_type_push_inserter(queue), split_option);
...
inline std::size_t parse_n(const InputIterator begin, const InputIterator end,
  ...
  if (1 == delimiters.size())
    split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]),
            begin, end, n, range_to_type_push_inserter(stack), split_option);
  else
    split_n(multiple_char_delimiter_predicate(delimiters), begin, end, n,
            range_to_type_push_inserter(stack), split_option);
...
inline std::size_t parse_n(
  ...
  if (1 == delimiters.size())
    split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]),
            begin, end, n, range_to_type_push_inserter(priority_queue),
            split_option);
  else
    split_n(multiple_char_delimiter_predicate(delimiters), begin, end, n,
...
inline std::size_t parse_n(const InputIterator begin, const InputIterator end,
  ...
  if (1 == delimiters.size())
    split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]),
            begin, end, n, range_to_ptr_type(out, insert_count), split_option);
  else
    split_n(multiple_char_delimiter_predicate(delimiters), begin, end, n,
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4, t5,
      t6, t7, t8, t9, t10, t11, t12, t13, t14, t15,
      typename details::ca_type<
          T16, typename details::is_stl_container<T12>::result_t>::type(t16));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      typename details::ca_type<
          T15, typename details::is_stl_container<T12>::result_t>::type(t15));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      typename details::ca_type<
          T14, typename details::is_stl_container<T12>::result_t>::type(t14));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      typename details::ca_type<
          T13, typename details::is_stl_container<T12>::result_t>::type(t13));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      t6, t7, t8, t9, t10, t11,
      typename details::ca_type<
          T12, typename details::is_stl_container<T12>::result_t>::type(t12));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      t6, t7, t8, t9, t10,
      typename details::ca_type<
          T11, typename details::is_stl_container<T11>::result_t>::type(t11));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      t6, t7, t8, t9,
      typename details::ca_type<
          T10, typename details::is_stl_container<T10>::result_t>::type(t10));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      t6, t7, t8,
      typename details::ca_type<
          T9, typename details::is_stl_container<T9>::result_t>::type(t9));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      t6, t7,
      typename details::ca_type<
          T8, typename details::is_stl_container<T8>::result_t>::type(t8));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      ...
      t6,
      typename details::ca_type<
          T7, typename details::is_stl_container<T7>::result_t>::type(t7));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4, t5,
      typename details::ca_type<
          T6, typename details::is_stl_container<T6>::result_t>::type(t6));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4,
      typename details::ca_type<
          T5, typename details::is_stl_container<T5>::result_t>::type(t5));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
  ...
  return parse(
      to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3,
      typename details::ca_type<
          T4, typename details::is_stl_container<T4>::result_t>::type(t4));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
                  T1& t1, T2& t2, T3& t3) {
  return parse(
      to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2,
      typename details::ca_type<
          T3, typename details::is_stl_container<T3>::result_t>::type(t3));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
                  T1& t1, T2& t2) {
  return parse(
      to_ptr(data), to_ptr(data) + data.size(), delimiters, t1,
      typename details::ca_type<
          T2, typename details::is_stl_container<T2>::result_t>::type(t2));
}
...
inline bool parse(const std::string& data, const std::string& delimiters,
                  T& t) {
  return parse(
      to_ptr(data), to_ptr(data) + data.size(), delimiters,
      typename details::ca_type<
          T, typename details::is_stl_container<T>::result_t>::type(t));
}
...
#define strtk_def_parse_cont(Type)                                           \
  ...
  inline std::size_t parse(const std::string& data,                          \
                           ...
                           const split_options::type& split_option =         \
                               split_options::compress_delimiters) {         \
    return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, cont, \
                 split_option);                                              \
  }
...
#undef strtk_def_parse_cont
        ...
        template <typename T, typename Comparator, typename Allocator>
        inline std::size_t
    parse(const std::string& data, const std::string& delimiters,
          std::set<T, Comparator, Allocator>& set,
          const split_options::type& split_option =
              split_options::compress_delimiters) {
  return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, set,
               split_option);
}
...
inline std::size_t parse(const std::string& data, const std::string& delimiters,
                         ...
                         const split_options::type& split_option =
                             split_options::compress_delimiters) {
  return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, multiset,
               split_option);
}
...
inline std::size_t parse(const std::string& data, const std::string& delimiters,
                         ...
                         const split_options::type& split_option =
                             split_options::compress_delimiters) {
  return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, queue,
               split_option);
}
...
inline std::size_t parse(const std::string& data, const std::string& delimiters,
                         ...
                         const split_options::type& split_option =
                             split_options::compress_delimiters) {
  return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, stack,
               split_option);
}
...
inline std::size_t parse(
    ...
    const split_options::type& split_option =
        split_options::compress_delimiters) {
  return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters,
...
#define strtk_cmpstmt(N)                                                    \
  if (!string_to_type_converter(std::string(argv[N]), t##N)) return result; \
  ++result;

  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
          strtk_cmpstmt(9) return result;
}
...

{
  ...
  std::size_t result = 0;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) strtk_cmpstmt(7) return result;
}
...

{
  ...
  std::size_t result = 0;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4)
      strtk_cmpstmt(5) strtk_cmpstmt(6) return result;
}
...
inline std::size_t parse(const int& argc, char* argv[], T1& t1, T2& t2,
  ...
  if (3 != argc) return 0;
  std::size_t result = 0;
  strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) return result;
}
...
inline std::size_t parse(const int& argc, char* argv[], T1& t1, T2& t2) {
  if (2 != argc) return 0;
  std::size_t result = 0;
  strtk_cmpstmt(1) strtk_cmpstmt(2) return result;
}
...
inline std::size_t parse(const int& argc, char* argv[], T1& t1) {
  if (1 != argc) return 0;
  std::size_t result = 0;
  strtk_cmpstmt(1) return result;
#undef strtk_cmpstmt
}

#define strtk_parse_begin(Type)                                      \
  namespace strtk {                                                  \
  bool parse(const std::string& data, const std::string& delimiters, \
             Type& t) { return parse(data,delimiters

#define strtk_parse_type(T) , t.T

#define strtk_parse_hex_type(T) , t.T

#define strtk_parse_ignore_token() , ignore_token()
...
#define strtk_def_parse_n_seq(Type)                                         \
  ...
  inline std::size_t parse_n(                                               \
      const std::string& data, const std::string& delimiters,               \
      const std::size_t& n, Type<T, Allocator>& sequence,                   \
      const split_options::type& split_option =                             \
          split_options::compress_delimiters) {                             \
    return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, \
                   sequence, split_option);                                 \
  }

strtk_def_parse_n_seq(std::vector) strtk_def_parse_n_seq(std::deque)
    strtk_def_parse_n_seq(std::list)
#undef strtk_def_parse_n_seq

        template <typename T, typename Comparator, typename Allocator>
        inline std::size_t
    parse_n(const std::string& data, const std::string& delimiters,
            const std::size_t& n, std::set<T, Comparator, Allocator>& set,
            const split_options::type& split_option =
                split_options::compress_delimiters) {
  return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, set,
                 split_option);
}
...
inline std::size_t parse_n(const std::string& data,
                           ...
                           const split_options::type& split_option =
                               split_options::compress_delimiters) {
  return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, queue,
                 split_option);
}
...
inline std::size_t parse_n(const std::string& data,
                           ...
                           const split_options::type& split_option =
                               split_options::compress_delimiters) {
  return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, stack,
                 split_option);
}
...
inline std::size_t parse_n(const std::string& data,
                           ...
                           const split_options::type& split_option =
                               split_options::compress_delimiters) {
  return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, out,
                 split_option);
}
...
inline bool parse_line(std::ifstream& stream, const std::string& delimiters,
  ...
  if (data.empty() || delimiters.empty()) return false;
  return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, t1,
...
inline std::size_t parse_line(std::ifstream& stream,
  ...
  std::string data;
  data.reserve(strtk::one_kilobyte);
  if (!std::getline(stream, data)) return 0;
  if (data.empty() || delimiters.empty()) return false;
  return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters,
                      sequence, split_option);
}
...
inline std::size_t parse_line(std::ifstream& stream,
  ...
  if (!std::getline(stream, data)) return 0;
  if (data.empty() || delimiters.empty()) return false;
  return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, set,
                      split_option);
}
...
template <typename T, typename Container, typename Comparator>
inline std::size_t parse_line(
    std::ifstream& stream, const std::string& delimiters,
    std::priority_queue<T, Container, Comparator>& priority_queue,
    const split_options::type& split_option =
        split_options::compress_delimiters)
...
inline std::size_t parse_line_n(std::ifstream& stream,
  ...
  return strtk::parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n,
                        sequence, split_option);
}
...
template <typename T, typename Comparator, typename Allocator>
inline std::size_t parse_line_n(
    std::ifstream& stream, const std::string& delimiters, const std::size_t& n,
    std::multiset<T, Comparator, Allocator>& multiset,
    const split_options::type& split_option =
        split_options::compress_delimiters) {
  if (!stream) return 0;
...
strtk_def_join_seq(std::vector) strtk_def_join_seq(std::deque)
    strtk_def_join_seq(std::list) strtk_def_join_seq(std::priority_queue)
...
template <typename T, typename Predicate, typename Comparator,
          typename Allocator>
inline void join_if(std::string& output, const std::string& delimiter,
                    Predicate predicate,
                    const std::set<T, Comparator, Allocator>& set) {
...
template <typename InputIterator, typename Predicate>
inline std::string join_if(
    const std::string& delimiter, Predicate predicate,
    const std::pair<InputIterator, InputIterator>& range) {
  std::string output;
  output.reserve(one_kilobyte);
...
class build_string {
 public:
  ...
  inline std::string to_str() const { return data_; }

  inline operator const char*() const { return data_.data(); }
...
strtk_def_brktz_cont(std::vector) strtk_def_brktz_cont(std::deque)
    strtk_def_brktz_cont(std::list) strtk_def_brktz_cont(std::queue)
        strtk_def_brktz_cont(std::stack)
#undef strtk_def_brktz_cont

            template <typename T, typename Comparator, typename Allocator>
            inline std::string
    bracketize(const std::string& pre, const std::string& post,
               std::set<T, Comparator, Allocator>& set) {
...
template <typename Comparator, typename Allocator>
inline void cut(
    const std::size_t& r0, const std::size_t& r1,
    const std::multiset<std::string, Comparator, Allocator>& multiset) {
...
template <typename T>
struct supported_random_type {};

#define strtk_register_rand_int_type_tag(T) \
  template <>                               \
  struct supported_random_type<T> {         \
    typedef rand_int_type_tag type;         \
    enum { value = true };                  \
  };
...
#define strtk_register_rand_real_type_tag(T) \
  template <>                                \
  struct supported_random_type<T> {          \
    typedef rand_real_type_tag type;         \
    enum { value = true };                   \
  };
...
strtk_register_rand_int_type_tag(char) strtk_register_rand_int_type_tag(
    ...
    strtk_register_rand_int_type_tag(short) strtk_register_rand_int_type_tag(
        int) strtk_register_rand_int_type_tag(long)
...
#undef strtk_register_rand_real_type_tag
                                ...
                                inline void generate_random_values_impl(
                                    const std::size_t& count, const T& min,
                                    const T& max, OutputIterator out,
                                    RandomNumberGenerator& rng,
                                    rand_int_type_tag) {
  // Note: The implied range will be: [min,max]
  using namespace boost;
  variate_generator<RandomNumberGenerator&, uniform_int<T> > rnd(
      rng, uniform_int<T>(min, max));
...
inline void generate_random_values_impl(const std::size_t& count, const T& min,
  ...
  // Note: The implied range will be: [min,max)
  using namespace boost;
  variate_generator<RandomNumberGenerator&, uniform_real<T> > rnd(
      rng, uniform_real<T>(min, max));
  for (std::size_t i = 0; i < count; ++i, *out++ = rnd())
    ;
}
...
class uniform_real_rng {
 ...
 public:
  ...
  inline double operator()() { return rnd_(); }
...
inline bool random_combination(const Iterator begin, const Iterator end,
                               std::size_t set_size, RandomNumberGenerator& rng,
                               OutputIterator out) {
  const std::size_t size = std::distance(begin, end);
...
inline std::size_t select_k_randomly(const Iterator begin, const Iterator end,
                                     const std::size_t k, OutputIterator out,
                                     RandomNumberGenerator& rng) {
...
inline bool for_each_permutation_conditional(Iterator begin, Iterator end,
                                             Function function) {
  do {
    if (!function(begin, end)) return false;
  } while (std::next_permutation(begin, end));
...
*/
template <typename Iterator>
static inline void rotate_discontinuous(
...
inline void combine_discontinuous(
    Iterator first1, Iterator last1,
    typename std::iterator_traits<Iterator>::difference_type d1,
    Iterator first2, Iterator last2,
    typename std::iterator_traits<Iterator>::difference_type d2, Function& f,
    typename std::iterator_traits<Iterator>::difference_type d = 0) {
  typedef typename std::iterator_traits<Iterator>::difference_type D;
  using std::swap;

  if ((0 == d1) || (0 == d2)) return f();
  if (1 == d1) {
    Iterator i2 = first2;
    while (i2 != last2) {
      f();
...
template <typename Iterator, class Function>
inline bool combine_discontinuous_conditional(
    Iterator first1, Iterator last1,
    typename std::iterator_traits<Iterator>::difference_type d1,
    Iterator first2, Iterator last2,
    typename std::iterator_traits<Iterator>::difference_type d2, Function& f,
  ...
  if (d1 == 1) {
    ...
    for (D d22 = d2; i2 != last2; ++i2, --d22) {
      if (!combine_discontinuous_conditional(f1p, last1, d1 - 1, i2, last2, d22,
                                             f, d + 1))
        return false;
      swap(*first1, *i2);
    }
...
class bound_range {
 public:
  ...
  inline void operator()() { f_(first_, last_); }
...
class bound_range_conditional {
 public:
  ...
  inline bool operator()() { return f_(first_, last_); }
...
inline void for_each_combination_conditional(Iterator begin, Iterator end,
  ...
  details::bound_range_conditional<Function&, Iterator> func(function, begin,
                                                             mid);
  details::combine_discontinuous_conditional(
      begin, mid, std::distance(begin, mid), mid, end, std::distance(mid, end),
      func);
}
...
inline unsigned long long int n_choose_k(const unsigned long long int& n,
  ...
  class n_choose_k_impl {
   public:
    ...
    inline value_type compute(const value_type& n, const value_type& k) {
      ...
      if (0 == v1) v1 = lookup(n - 1, k - 1) = compute(n - 1, k - 1);

      value_type v2 = lookup(n - 1, k);

      if (0 == v2) v2 = lookup(n - 1, k) = compute(n - 1, k);
   ...
   private:
    inline n_choose_k_impl& operator=(const n_choose_k_impl&) { return *this; }
  };
  ...
  const std::size_t table_size =
      static_cast<std::size_t>(n * (n / 2) + (n & 1));
  ...
  if (table != static_table) delete[] table;
...
inline void nth_combination_sequence(unsigned long long int n,
                                     const std::size_t& r, const std::size_t& k,
                                     OutputIterator out,
...
class combination_iterator
 ...
 public:
  ...
  inline range_type operator*() const { return current_combination_; }
...
struct all_digits_check_impl<Iterator, 0> {
  static inline bool process(Iterator) { return true; }
};
...
struct numeric_convert_impl<T, Iterator, 20> {
  static inline void process(const Iterator itr, T& t) {
    strtk::fast::details::numeric_convert_impl<T, Iterator, 19>::process(
        itr + 1, t);
...
struct numeric_convert_impl<T, Iterator, 2> {
  static inline void process(const Iterator itr, T& t) {
    t = static_cast<T>(itr[0] - '0') * 10 + static_cast<T>(itr[1] - '0');
  }
};
...
struct numeric_convert_impl<T, Iterator, 0> {
  static inline void process(const Iterator, T& t) { t = 0; }
};
...
#define case_stmt(N) \
  case N:            \
    ...
    case_stmt(0) case_stmt(1) case_stmt(2) case_stmt(3) case_stmt(4)
        case_stmt(5) case_stmt(6) case_stmt(7) case_stmt(8) case_stmt(9)
            case_stmt(10) case_stmt(11) case_stmt(12) case_stmt(13)
                case_stmt(14) case_stmt(15) case_stmt(16) case_stmt(17)
                    case_stmt(18) case_stmt(19) default : return false;
#undef case_stmt
  }
}
...
template <typename T, typename Iterator>
inline bool numeric_convert(const std::size_t n, const Iterator itr, T& t,
                            const bool digit_check = false) {
  if (digit_check) {
  ...
  case N: {                                                         \
    ...
    case_stmt(0) case_stmt(1) case_stmt(2) case_stmt(3) case_stmt(4)
        case_stmt(5) case_stmt(6) case_stmt(7) case_stmt(8) case_stmt(9)
            case_stmt(10) case_stmt(11) case_stmt(12) case_stmt(13)
                case_stmt(14) case_stmt(15) case_stmt(16) case_stmt(17)
                    case_stmt(18) case_stmt(19) case_stmt(20) default
        : return false;
#undef case_stmt
  }
}
...
template <typename T, typename Iterator>
inline bool signed_numeric_convert(Iterator itr, const Iterator end, T& t,
                                   const bool digit_check = false) {
...
inline bool signed_numeric_convert(const std::string& s, T& t,
                                   const bool digit_check = false) {
  return signed_numeric_convert<T, const char*>(to_ptr(s), to_ptr(s) + s.size(),
                                                t, digit_check);
}
...
#if (defined(__LITTLE_ENDIAN__)) || (defined(WIN32)) || (defined(_WIN32)) || \
    (defined(__WIN32__)) || (defined(__MINGW32_VERSION)) ||                  \
...
static inline bool is_big_endian() { return details_endian::__be_result; }
...
static inline long long int convert(const long long int v) {
  return static_cast<long long>(
      convert(static_cast<unsigned long long int>(v)));
}
...
class reader {
 public:
  ...
  inline std::size_t position() const { return amount_read_sofar_; }

  inline const char* position_ptr() const { return buffer_; }

  inline std::size_t amount_read() { return amount_read_sofar_; }
  ...
  inline bool operator()(T*& data, uint32_t& length,
    ...
    if (read_length && !operator()(length)) return false;

    const std::size_t raw_size = length * sizeof(T);
    if (!buffer_capacity_ok(raw_size)) return false;
  ...
  inline bool operator()(T*& data, uint64_t& length,
                         const bool read_length = true) {
    uint32_t l = 0;
    if (read_length) l = static_cast<uint32_t>(length);
    if (!operator()(data, l, read_length)) return false;
    if (read_length) length = l;
    return true;
  }
  ...
  inline bool operator()(std::string& output) {
    ...
    if (!buffer_capacity_ok(length)) return false;

    output.resize(length);
    std::copy(buffer_, buffer_ + length, const_cast<char*>(to_ptr(output)));
  ...
  inline bool operator()(std::pair<T1, T2>& p) {
    if (!operator()(p.first)) return false;
    if (!operator()(p.second)) return false;
    return true;
  }
...
#define strtk_def_rdr_opr(Type)                      \
  ...
  strtk_def_rdr_opr(std::deque) strtk_def_rdr_opr(std::queue)
...
#undef strtk_def_rdr_opr
  ...
  inline bool operator()(T (&output)[N]) {
    ...
    if (buffer_capacity_ok(raw_size)) {
      std::copy(buffer_, buffer_ + raw_size, reinterpret_cast<char*>(output));
  ...
  inline bool operator()(T& output, const std::size_t& size) {
    if (buffer_capacity_ok(size)) {
      bool result = strtk::string_to_type_converter<char*, T>(
          buffer_, buffer_ + size, output);
      buffer_ += size;
  ...
  inline void mark() { marker_.mark(amount_read_sofar_, buffer_); }
 ...
 private:
  ...
  struct selector {
   private:
    ...
    struct selector_impl {
      ...
      static inline bool batch_vector_read(Reader& r, const std::size_t& size,
  ...
  inline bool read_pod(T& data,
    ...
    if (perform_buffer_capacity_check) {
      if (!buffer_capacity_ok(data_length)) return false;
    }
...
class writer {
 public:
  ...
  inline std::size_t position() const { return amount_written_sofar_; }

  inline const char* position_ptr() const { return buffer_; }

  inline std::size_t amount_written() const { return amount_written_sofar_; }
  ...
  inline bool operator()(const T (&data)[N], const bool write_length = false) {
    if (write_length && !operator()(N)) return false;
  ...
  inline bool operator()(const T* data, const uint32_t& length,
    ...
    if (write_length && !operator()(length)) return false;
...
#define strtk_def_wrtr_opr(Type)                                   \
  ...
  strtk_def_wrtr_opr(std::deque) strtk_def_wrtr_opr(std::list)
...
#undef strtk_def_wrtr_opr
    ...
    const std::size_t raw_size = (size * sizeof(T));
    if (!buffer_capacity_ok(raw_size + sizeof(size))) return false;
    if (!operator()(size)) return false;
  ...
  enum padding_mode { right_padding = 0, left_padding = 1 };
  ...
  inline bool operator()(const T& input, const std::size_t& size,
    ...
    if (amount_written_sofar_ + size <= buffer_length_) {
      ...
      return operator()<const char>(to_ptr(s), static_cast<uint32_t>(size),
                                    false);
    } else
      return false;
  }

  inline void mark() { marker_.mark(amount_written_sofar_, buffer_); }
  ...
  struct selector {
   private:
    ...
    struct selector_impl {
      ...
      static inline bool batch_vector_writer(
          Writer& w, const std::size_t&, const std::vector<T, Allocator>& v) {
    ...
    struct selector_impl<T, strtk::details::yes_t> {
      ...
      template <typename Writer, typename Allocator>
      static inline bool batch_vector_writer(
          Writer& w, const std::size_t& raw_size,
...
#define strtk_binary_reader(T) &&reader(T)
...
#define strtk_binary_writer(T) &&writer(T)
...
class short_string_impl {
 public:
  short_string_impl() : s(0) {}

  short_string_impl(std::string& str) : s(&str) {}
  ...
  inline bool operator()(reader& r) {
    ...
    if (!r(ptr, length, false)) return false;
    return true;
  }

  inline bool operator()(writer& w) const {
    if (0 == s) return false;
    if (s->size() > std::numeric_limits<size_type>::max()) return false;
    const size_type size = static_cast<size_type>(s->size());
    if (!w(size)) return false;
    if (!w(s->data(), size, false)) return false;
    return true;
  }
...
class ignore_token {
 public:
  ...
  inline ignore_token& operator=(const std::string&) { return (*this); }
};

template <typename T>
class hex_to_number_sink {
  // static_assert for T either int or unsigned int and alike (could use a
  // concept)
 private:
 ...
 public:
  hex_to_number_sink(T& t) : valid_(false), t_(&t) {}
  ...
  inline hex_to_number_sink& operator=(
    ...
    if ((size > 2) && ((*s.first) == '0') &&
      ...
      offset = 2;
    if ((size - offset) > (2 * sizeof(T))) return (*this);
    ...
    (*t_) = 0;
    valid_ =
        convert_hex_to_bin(buffer, buffer + (size - offset) + buffer_offset,
...
class base64_to_number_sink {
 ...
 public:
  base64_to_number_sink(T& t) : valid_(false), t_(&t) {}
  ...
  inline base64_to_number_sink& operator=(const std::string& s) {
    if (!range_only_contains(base64_value_check(), to_ptr(s),
                             to_ptr(s) + s.size())) {
      valid_ = false;
      return (*this);
    }
...
class hex_to_string_sink {
 public:
  hex_to_string_sink(std::string& s) : valid_(false), s_(s) {}
  ...
  inline hex_to_string_sink& operator=(const std::string& s) {
    return this->operator=(std::make_pair<char*>(
        const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size())));
  }
  ...
  inline bool operator()(InputIterator begin, InputIterator end) {
    ...
    if (std::numeric_limits<std::size_t>::max() != fractional_size_) {
      if (size < (fractional_size_ + 1))
        ...
        return strtk::string_to_type_converter(
            begin, begin + (size - (fractional_size_ + 1)), (*t_));
    }
...
class decimal_sink {
 public:
  decimal_sink(const std::size_t& int_size, const std::size_t& frac_size)
      : t_(0), int_size_(int_size), frac_size_(frac_size) {}

  decimal_sink(T& t, const std::size_t& int_size, const std::size_t& frac_size)
      : t_(&t), int_size_(int_size), frac_size_(frac_size) {}
...
#define strtk_def_parse_stl_cnt_prxy(Type)                          \
  template <typename InputIterator, typename T, typename Allocator> \
  inline std::size_t parse_stl_container_proxy(                     \
      const InputIterator begin, const InputIterator end,           \
      const std::string& delimiters, Type<T, Allocator>& sequence,  \
      const split_options::type& split_option =                     \
          split_options::compress_delimiters) {                     \
...
#undef strtk_def_stl_cnt_prxy
                        ...
                        inline std::size_t
    parse_stl_container_proxy(const InputIterator begin,
                              const InputIterator end,
                              const std::string& delimiters,
                              std::set<T, Comparator, Allocator>& set,
                              const split_options::type& split_option =
...
#define strtk_def_parse_n_stl_cnt_prxy(Type)                           \
  template <typename InputIterator, typename T, typename Allocator>    \
  inline std::size_t parse_n_stl_container_proxy(                      \
      const InputIterator begin, const InputIterator end,              \
      const std::string& delimiters, const std::size_t& n,             \
      Type<T, Allocator>& sequence,                                    \
      const split_options::type& split_option =                        \
...
#undef strtk_def_parse_n_stl_cnt_prxy
                        ...
                        template <typename InputIterator, typename T,
                                  typename Comparator, typename Allocator>
                        inline std::size_t
    parse_n_stl_container_proxy(const InputIterator begin,
                                const InputIterator end,
                                const std::string& delimiters,
                                const std::size_t& n,
                                std::set<T, Comparator, Allocator>& set,
 ...
 public:
  ...
  inline sink_type& operator()(Container& container,
                               ...
                               const split_options::type& split_option =
                                   split_options::compress_delimiters) {
    container_ = (&container);
    if (!delimiters.empty()) delimiters_ = delimiters;
    split_option_ = split_option;
    return (*this);
  }
  ...
  sink_type<Container>& reference() { return *this; }
...
inline bool match_exactly_n_consecutive_values(const std::size_t n, Predicate p,
                                               Iterator itr,
                                               const Iterator end) {
  if (static_cast<unsigned int>(std::distance(itr, end)) < n) return false;
...
inline bool match_n_consecutive_values(const std::size_t n,
                                       find_mode::type mode, Predicate p,
...
inline typename details::range_type<Iterator>::type find_n_consecutive(
  ...
  switch (type) {
    case find_type::digits:
      return details::find_n_consecutive_values<Iterator>(
          n, mode, strtk::text::is_digit, range.first, range.second);

    case find_type::letters:
      return details::find_n_consecutive_values<Iterator>(
          n, mode, strtk::text::is_letter, range.first, range.second);

    case find_type::lowercase_letters:
      return details::find_n_consecutive_values<Iterator>(
          n, mode, strtk::text::is_lowercase_letter, range.first, range.second);

    case find_type::uppercase_letters:
      return details::find_n_consecutive_values<Iterator>(
          n, mode, strtk::text::is_uppercase_letter, range.first, range.second);

    case find_type::letters_digits:
      return details::find_n_consecutive_values<Iterator>(
          n, mode, strtk::text::is_letter_or_digit, range.first, range.second);
...
template <typename Iterator>
inline bool match_n_consecutive(
    const std::size_t n, find_type::type type, find_mode::type mode,
    typename details::range_type<Iterator>::type range) {
  switch (type) {
    case find_type::digits:
      return details::match_n_consecutive_values<Iterator>(
          n, mode, strtk::text::is_digit, range.first, range.second);
...
inline std::size_t split_on_consecutive(const std::size_t n, Predicate p,
  ...
  while (end != itr_range.first) {
    range_type found_itr =
        details::find_exactly_n_consecutive_values<iterator_type, Predicate>(
            n, p, itr_range, stateful_predicate);
...
inline std::size_t split_on_consecutive(const std::size_t n,
                                        const std::size_t m, Predicate p,
                                        char* begin, char* end,
                                        OutputIterator out) {
  if (0 == n) return 0;
  ...
  while ((end != itr_range.first) && (match_count <= n)) {
    range_type found_itr =
        details::find_exactly_n_consecutive_values(m, p, itr_range);
    if ((end == found_itr.first) && (found_itr.first == found_itr.second)) {
      break;
    } else {
      (*out) = found_itr;
...
inline std::size_t split_on_consecutive(const std::size_t& n,
                                        ...
                                        const find_mode::type mode, char* begin,
  ...
  while (end != itr_range.first) {
    range_type found_itr =
        find_n_consecutive<iterator_type>(n, type, mode, itr_range);
...
template <typename InputIterator, typename OutputIterator>
inline std::size_t split_on_consecutive_n(
    const std::size_t& n, const std::size_t& m, const find_type::type type,
    const find_mode::type mode, char* begin, char* end, OutputIterator out) {
  if (0 == n) return 0;
  ...
  while ((end != itr_range.first) && (match_count <= n)) {
    range_type found_itr =
        find_n_consecutive<iterator_type>(m, type, mode, itr_range);
...
inline std::size_t split_on_consecutive(const std::size_t& n,
                                        ...
                                        OutputIterator out) {
  return split_on_consecutive<char*, OutputIterator>(
      n, type, mode, const_cast<char*>(begin), const_cast<char*>(end), out);
}
...
inline std::size_t split_on_consecutive(const std::size_t& n,
                                        ...
                                        const unsigned char* end,
                                        OutputIterator out) {
  return split_on_consecutive<OutputIterator>(
      n, type, mode, reinterpret_cast<const char*>(begin),
...
inline std::size_t split_on_consecutive(const std::size_t& n,
                                        ...
                                        const std::string& str,
                                        OutputIterator out) {
  return split_on_consecutive<OutputIterator>(n, type, mode, to_ptr(str),
                                              to_ptr(str) + str.size(), out);
}
...
inline std::size_t split_on_consecutive_n(const std::size_t& n,
                                          ...
                                          const char* begin, const char* end,
                                          OutputIterator out) {
  return split_on_consecutive_n<char*, OutputIterator>(
      n, m, type, mode, const_cast<char*>(begin), const_cast<char*>(end), out);
}
...
inline std::size_t split_on_consecutive_n(
  ...
  return split_on_consecutive_n<OutputIterator>(
      n, m, type, mode, reinterpret_cast<const char*>(begin),
      reinterpret_cast<const char*>(end), out);
}
...
inline std::size_t split_on_consecutive_n(
    const std::size_t& n, const std::size_t& m, const find_type::type type,
    const find_mode::type mode, const std::string& str, OutputIterator out) {
  return split_on_consecutive_n<OutputIterator>(n, m, type, mode, to_ptr(str),
                                                to_ptr(str) + str.size(), out);
}
...
inline std::size_t split_on_consecutive(const std::size_t& n, Predicate p,
                                        ...
                                        OutputIterator out,
                                        const bool stateful_predicate = false) {
  return split_on_consecutive<Predicate, OutputIterator>(
      n, p, const_cast<char*>(begin), const_cast<char*>(end), out,
      stateful_predicate);
}
...
inline std::size_t split_on_consecutive(const std::size_t& n, Predicate p,
                                        ...
                                        const bool stateful_predicate = false) {
  return split_on_consecutive<Predicate, OutputIterator>(
      n, p, reinterpret_cast<const char*>(begin),
...
inline std::size_t split_on_consecutive(const std::size_t& n, Predicate p,
                                        ...
                                        const bool stateful_predicate = false) {
  return split_on_consecutive<Predicate, OutputIterator>(
      n, p, to_ptr(str), to_ptr(str) + str.size(), out, stateful_predicate);
}
...
inline std::size_t split_on_consecutive_n(const std::size_t& n,
                                          ...
                                          const char* begin, const char* end,
                                          OutputIterator out) {
  return split_on_consecutive_n<Predicate, char*, OutputIterator>(
      n, m, p, const_cast<char*>(begin), const_cast<char*>(end), out);
}
...
inline std::size_t split_on_consecutive_n(const std::size_t& n,
                                          ...
                                          const unsigned char* end,
                                          OutputIterator out) {
  return split_on_consecutive_n<Predicate, OutputIterator>(
      n, m, p, reinterpret_cast<const char*>(begin),
      reinterpret_cast<const char*>(end), out);
}
...
inline std::size_t split_on_consecutive_n(const std::size_t& n,
                                          ...
                                          OutputIterator out) {
  return split_on_consecutive_n<Predicate, OutputIterator>(
      n, m, p, to_ptr(str), to_ptr(str) + str.size(), out);
}
...
class expect_impl {
 public:
  expect_impl(const std::string& s) : s_(s) {}
  ...
  inline expect_impl& ref() { return (*this); }

  inline void set_value(const std::string& s) { s_ = s; }
...
class iexpect_impl {
 public:
  iexpect_impl(const std::string& s) : s_(s) {}
  ...
  inline iexpect_impl& ref() { return (*this); }
...
class like_impl {
 public:
  like_impl(const std::string& s) : s_(s) {}
  ...
  inline like_impl& ref() { return (*this); }

  inline void set_pattern(const std::string& s) { s_ = s; }
...
class inrange_impl {
 public:
  inrange_impl(T& t, const T& low, const T& hi) : t_(&t), low_(low), hi_(hi) {}
  ...
  inline inrange_impl<T>& ref() { return (*this); }
...
struct convert_impl {
  ...
  static bool execute(InputIterator begin, InputIterator end,
    ...
    std::string s;
    if (!strtk::string_to_type_converter(begin, end, s)) return false;
    switch (mode) {
      case 0:
...
struct convert_impl<std::string> {
  template <typename InputIterator>
  static bool execute(InputIterator begin, InputIterator end,
                      const std::string& rem_chars, std::size_t mode,
                      std::string& t) {
    if (!strtk::string_to_type_converter(begin, end, t)) return false;
...
class trim_impl {
 public:
  ...
  inline trim_impl<T>& ref() { return (*this); }
...
class conv_to_lcase_impl {
 public:
  conv_to_lcase_impl(std::string& s) : s_(&s) {}
  ...
  inline conv_to_lcase_impl& ref() { return (*this); }
...
class conv_to_ucase_impl {
 public:
  conv_to_ucase_impl(std::string& s) : s_(&s) {}
  ...
  inline conv_to_ucase_impl& ref() { return (*this); }
...
class fill_array_impl {
 public:
  ...
  inline bool operator()(InputIterator begin, InputIterator end) {
    ...
    if (range_size != size_) return false;
    std::memcpy(data_, begin, range_size);
    return true;
  }

  inline fill_array_impl& ref() { return (*this); }
...
static const std::size_t digit_table_size =
    sizeof(digit_table) / sizeof(unsigned char);
...
static const unsigned char rev_3digit_lut[] = {
    ...
    "4"
    "02502602702802903003103203303403503603703803904004104204304404504604704804"
    "9"
    "05005105205305405505605705805906006106206306406506606706806907007107207307"
    "4"
    "07507607707807908008108208308408508608708808909009109209309409509609709809"
    "9"
    "10010110210310410510610710810911011111211311411511611711811912012112212312"
    "4"
    "12512612712812913013113213313413513613713813914014114214314414514614714814"
    "9"
    "15015115215315415515615715815916016116216316416516616716816917017117217317"
    "4"
    "17517617717817918018118218318418518618718818919019119219319419519619719819"
    "9"
    "20020120220320420520620720820921021121221321421521621721821922022122222322"
    "4"
    "22522622722822923023123223323423523623723823924024124224324424524624724824"
    "9"
    "25025125225325425525625725825926026126226326426526626726826927027127227327"
    "4"
    "27527627727827928028128228328428528628728828929029129229329429529629729829"
    "9"
    "30030130230330430530630730830931031131231331431531631731831932032132232332"
    "4"
    "32532632732832933033133233333433533633733833934034134234334434534634734834"
    "9"
    "35035135235335435535635735835936036136236336436536636736836937037137237337"
    "4"
    "37537637737837938038138238338438538638738838939039139239339439539639739839"
    "9"
    "40040140240340440540640740840941041141241341441541641741841942042142242342"
    "4"
    "42542642742842943043143243343443543643743843944044144244344444544644744844"
    "9"
    "45045145245345445545645745845946046146246346446546646746846947047147247347"
    "4"
    "47547647747847948048148248348448548648748848949049149249349449549649749849"
    "9"
    "50050150250350450550650750850951051151251351451551651751851952052152252352"
    "4"
    "52552652752852953053153253353453553653753853954054154254354454554654754854"
    "9"
    "55055155255355455555655755855956056156256356456556656756856957057157257357"
    "4"
    "57557657757857958058158258358458558658758858959059159259359459559659759859"
    "9"
    "60060160260360460560660760860961061161261361461561661761861962062162262362"
    "4"
    "62562662762862963063163263363463563663763863964064164264364464564664764864"
    "9"
    "65065165265365465565665765865966066166266366466566666766866967067167267367"
    "4"
    "67567667767867968068168268368468568668768868969069169269369469569669769869"
    "9"
    "70070170270370470570670770870971071171271371471571671771871972072172272372"
    "4"
    "72572672772872973073173273373473573673773873974074174274374474574674774874"
    "9"
    "75075175275375475575675775875976076176276376476576676776876977077177277377"
    "4"
    "77577677777877978078178278378478578678778878979079179279379479579679779879"
    "9"
    "80080180280380480580680780880981081181281381481581681781881982082182282382"
    "4"
    "82582682782882983083183283383483583683783883984084184284384484584684784884"
    "9"
    "85085185285385485585685785885986086186286386486586686786886987087187287387"
    "4"
    "87587687787887988088188288388488588688788888989089189289389489589689789889"
    "9"
    "90090190290390490590690790890991091191291391491591691791891992092192292392"
    "4"
    "92592692792892993093193293393493593693793893994094194294394494594694794894"
    "9"
    "95095195295395495595695795895996096196296396496596696796896997097197297397"
    "4"
    "97597697797897998098198298398498598698798898999099199299399499599699799899"
    "9"
...
inline T pow10(T d, const int exponent) {
  static const double fract10[] = {
      0.0,      1.0E+001, 1.0E+002, 1.0E+003, 1.0E+004, 1.0E+005, 1.0E+006,
      1.0E+007, 1.0E+008, 1.0E+009, 1.0E+010, 1.0E+011, 1.0E+012, 1.0E+013,
      1.0E+014, 1.0E+015, 1.0E+016, 1.0E+017, 1.0E+018, 1.0E+019, 1.0E+020,
      1.0E+021, 1.0E+022, 1.0E+023, 1.0E+024, 1.0E+025, 1.0E+026, 1.0E+027,
      1.0E+028, 1.0E+029, 1.0E+030, 1.0E+031, 1.0E+032, 1.0E+033, 1.0E+034,
      1.0E+035, 1.0E+036, 1.0E+037, 1.0E+038, 1.0E+039, 1.0E+040, 1.0E+041,
      1.0E+042, 1.0E+043, 1.0E+044, 1.0E+045, 1.0E+046, 1.0E+047, 1.0E+048,
      1.0E+049, 1.0E+050, 1.0E+051, 1.0E+052, 1.0E+053, 1.0E+054, 1.0E+055,
      1.0E+056, 1.0E+057, 1.0E+058, 1.0E+059, 1.0E+060, 1.0E+061, 1.0E+062,
      1.0E+063, 1.0E+064, 1.0E+065, 1.0E+066, 1.0E+067, 1.0E+068, 1.0E+069,
      1.0E+070, 1.0E+071, 1.0E+072, 1.0E+073, 1.0E+074, 1.0E+075, 1.0E+076,
      1.0E+077, 1.0E+078, 1.0E+079, 1.0E+080, 1.0E+081, 1.0E+082, 1.0E+083,
      1.0E+084, 1.0E+085, 1.0E+086, 1.0E+087, 1.0E+088, 1.0E+089, 1.0E+090,
      1.0E+091, 1.0E+092, 1.0E+093, 1.0E+094, 1.0E+095, 1.0E+096, 1.0E+097,
      1.0E+098, 1.0E+099, 1.0E+100, 1.0E+101, 1.0E+102, 1.0E+103, 1.0E+104,
      1.0E+105, 1.0E+106, 1.0E+107, 1.0E+108, 1.0E+109, 1.0E+110, 1.0E+111,
      1.0E+112, 1.0E+113, 1.0E+114, 1.0E+115, 1.0E+116, 1.0E+117, 1.0E+118,
      1.0E+119, 1.0E+120, 1.0E+121, 1.0E+122, 1.0E+123, 1.0E+124, 1.0E+125,
      1.0E+126, 1.0E+127, 1.0E+128, 1.0E+129, 1.0E+130, 1.0E+131, 1.0E+132,
      1.0E+133, 1.0E+134, 1.0E+135, 1.0E+136, 1.0E+137, 1.0E+138, 1.0E+139,
      1.0E+140, 1.0E+141, 1.0E+142, 1.0E+143, 1.0E+144, 1.0E+145, 1.0E+146,
      1.0E+147, 1.0E+148, 1.0E+149, 1.0E+150, 1.0E+151, 1.0E+152, 1.0E+153,
      1.0E+154, 1.0E+155, 1.0E+156, 1.0E+157, 1.0E+158, 1.0E+159, 1.0E+160,
      1.0E+161, 1.0E+162, 1.0E+163, 1.0E+164, 1.0E+165, 1.0E+166, 1.0E+167,
      1.0E+168, 1.0E+169, 1.0E+170, 1.0E+171, 1.0E+172, 1.0E+173, 1.0E+174,
      1.0E+175, 1.0E+176, 1.0E+177, 1.0E+178, 1.0E+179, 1.0E+180, 1.0E+181,
      1.0E+182, 1.0E+183, 1.0E+184, 1.0E+185, 1.0E+186, 1.0E+187, 1.0E+188,
      1.0E+189, 1.0E+190, 1.0E+191, 1.0E+192, 1.0E+193, 1.0E+194, 1.0E+195,
      1.0E+196, 1.0E+197, 1.0E+198, 1.0E+199, 1.0E+200, 1.0E+201, 1.0E+202,
      1.0E+203, 1.0E+204, 1.0E+205, 1.0E+206, 1.0E+207, 1.0E+208, 1.0E+209,
      1.0E+210, 1.0E+211, 1.0E+212, 1.0E+213, 1.0E+214, 1.0E+215, 1.0E+216,
      1.0E+217, 1.0E+218, 1.0E+219, 1.0E+220, 1.0E+221, 1.0E+222, 1.0E+223,
      1.0E+224, 1.0E+225, 1.0E+226, 1.0E+227, 1.0E+228, 1.0E+229, 1.0E+230,
      1.0E+231, 1.0E+232, 1.0E+233, 1.0E+234, 1.0E+235, 1.0E+236, 1.0E+237,
      1.0E+238, 1.0E+239, 1.0E+240, 1.0E+241, 1.0E+242, 1.0E+243, 1.0E+244,
      1.0E+245, 1.0E+246, 1.0E+247, 1.0E+248, 1.0E+249, 1.0E+250, 1.0E+251,
      1.0E+252, 1.0E+253, 1.0E+254, 1.0E+255, 1.0E+256, 1.0E+257, 1.0E+258,
      1.0E+259, 1.0E+260, 1.0E+261, 1.0E+262, 1.0E+263, 1.0E+264, 1.0E+265,
      1.0E+266, 1.0E+267, 1.0E+268, 1.0E+269, 1.0E+270, 1.0E+271, 1.0E+272,
      1.0E+273, 1.0E+274, 1.0E+275, 1.0E+276, 1.0E+277, 1.0E+278, 1.0E+279,
      1.0E+280, 1.0E+281, 1.0E+282, 1.0E+283, 1.0E+284, 1.0E+285, 1.0E+286,
      1.0E+287, 1.0E+288, 1.0E+289, 1.0E+290, 1.0E+291, 1.0E+292, 1.0E+293,
      1.0E+294, 1.0E+295, 1.0E+296, 1.0E+297, 1.0E+298, 1.0E+299, 1.0E+300,
      1.0E+301, 1.0E+302, 1.0E+303, 1.0E+304, 1.0E+305, 1.0E+306, 1.0E+307,
      1.0E+308};

  static const int fract10_size =
      static_cast<int>(sizeof(fract10) / sizeof(double));
...
strtk_register_pod_type(bool) strtk_register_pod_type(signed char)
    strtk_register_pod_type(char) strtk_register_pod_type(short)
        strtk_register_pod_type(int) strtk_register_pod_type(long int)
...
template <typename ld, std::size_t size>
struct ldt {};
template <>
struct ldt<long double, sizeof(double)> {
  enum { i = -308, a = +308, p = 15 };
};  // 64-bit
template <>
struct ldt<long double, 10> {
  enum { i = -4931, a = +4931, p = 18 };
};  // 80-bit
template <>
struct ldt<long double, 12> {
  enum { i = -4931, a = +4931, p = 22 };
};  // 96-bit
template <>
struct ldt<long double, 2 * sizeof(double)> {
...
#define strtk_register_unsigned_type_tag(T)  \
  template <>                                \
  struct supported_conversion_to_type<T> {   \
    typedef unsigned_type_tag type;          \
  };                                         \
  template <>                                \
  struct supported_conversion_from_type<T> { \
    typedef unsigned_type_tag type;          \
  };
...
template <>
struct supported_conversion_to_type<hex_to_string_sink> {
...
#define strtk_register_supported_iterator_type(T) \
  template <>                                     \
  struct supported_iterator_type<T> {             \
    enum { value = true };                        \
  };

template <>
struct supported_conversion_to_type<bool> {
  typedef bool_type_tag type;
};
template <>
struct supported_iterator_type<bool> {
  enum { value = true };
};

template <>
struct supported_conversion_to_type<std::string> {
  typedef stdstring_type_tag type;
};
template <>
struct supported_iterator_type<std::string> {
  enum { value = true };
};

template <>
struct supported_conversion_to_type<strtk::util::value> {
  typedef value_type_tag type;
};
template <>
struct supported_conversion_from_type<strtk::util::value> {
  typedef value_type_tag type;
};
template <>
struct supported_iterator_type<strtk::util::value> {
  enum { value = true };
};

template <>
struct supported_conversion_to_type<strtk::details::expect_impl> {
  typedef expect_type_tag type;
};
template <>
struct supported_iterator_type<strtk::details::expect_impl> {
  enum { value = true };
};

template <>
struct supported_conversion_to_type<strtk::details::iexpect_impl> {
  typedef expect_type_tag type;
};
template <>
struct supported_iterator_type<strtk::details::iexpect_impl> {
  enum { value = true };
};

template <>
struct supported_conversion_to_type<strtk::details::like_impl> {
  typedef like_type_tag type;
};
template <>
struct supported_iterator_type<strtk::details::like_impl> {
  enum { value = true };
};

template <>
struct supported_conversion_to_type<strtk::details::fill_array_impl> {
  typedef fillchararray_type_tag type;
};
template <>
struct supported_iterator_type<strtk::details::fill_array_impl> {
  enum { value = true };
};

template <>
struct supported_conversion_to_type<strtk::details::conv_to_lcase_impl> {
  typedef lcase_type_tag type;
};
template <>
struct supported_iterator_type<strtk::details::conv_to_lcase_impl> {
  enum { value = true };
};

template <>
struct supported_conversion_to_type<strtk::details::conv_to_ucase_impl> {
  typedef ucase_type_tag type;
};
template <>
struct supported_iterator_type<strtk::details::conv_to_ucase_impl> {
  enum { value = true };
};

#define strtk_register_truncint_type_tag(T)                       \
  template <>                                                     \
  struct supported_conversion_to_type<strtk::truncated_int<T> > { \
    typedef truncint_type_tag type;                               \
  };                                                              \
  template <>                                                     \
  struct supported_iterator_type<strtk::truncated_int<T> > {      \
    enum { value = true };                                        \
  };

#define strtk_register_decsink_type_tag(T)                       \
  template <>                                                    \
  struct supported_conversion_to_type<strtk::decimal_sink<T> > { \
    typedef decsink_type_tag type;                               \
  };                                                             \
  template <>                                                    \
  struct supported_iterator_type<strtk::decimal_sink<T> > {      \
    enum { value = true };                                       \
  };

#define strtk_register_inrange_type_tag(T)                                \
  template <>                                                             \
  struct supported_conversion_to_type<strtk::details::inrange_impl<T> > { \
    typedef inrange_type_tag type;                                        \
  };                                                                      \
  template <>                                                             \
  struct supported_iterator_type<strtk::details::inrange_impl<T> > {      \
    enum { value = true };                                                \
  };

#define strtk_register_trim_type_tag(T)                                \
  template <>                                                          \
  struct supported_conversion_to_type<strtk::details::trim_impl<T> > { \
    typedef trim_type_tag type;                                        \
  };                                                                   \
  template <>                                                          \
  struct supported_iterator_type<strtk::details::trim_impl<T> > {      \
    enum { value = true };                                             \
  };

#define strtk_register_stdstring_range_type_tag(T)        \
  template <>                                             \
  struct supported_conversion_to_type<std::pair<T, T> > { \
    typedef stdstring_range_type_tag type;                \
  };

#define strtk_register_sink_type_tag(T)                                        \
  template <>                                                                  \
  struct supported_conversion_to_type<sink_type<std::vector<T> > > {           \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_to_type<sink_type<std::deque<T> > > {            \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_to_type<sink_type<std::list<T> > > {             \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_to_type<sink_type<std::set<T> > > {              \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_to_type<sink_type<std::multiset<T> > > {         \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_to_type<sink_type<std::queue<T> > > {            \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_to_type<sink_type<std::stack<T> > > {            \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_to_type<sink_type<std::priority_queue<T> > > {   \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_from_type<sink_type<std::vector<T> > > {         \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_from_type<sink_type<std::deque<T> > > {          \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_from_type<sink_type<std::list<T> > > {           \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_from_type<sink_type<std::set<T> > > {            \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_from_type<sink_type<std::multiset<T> > > {       \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_from_type<sink_type<std::queue<T> > > {          \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_from_type<sink_type<std::stack<T> > > {          \
    typedef sink_type_tag type;                                                \
  };                                                                           \
  template <>                                                                  \
  struct supported_conversion_from_type<sink_type<std::priority_queue<T> > > { \
    typedef sink_type_tag type;                                                \
  };

#define strtk_register_stl_container_to_string_conv_type_tag(T)    \
  template <>                                                      \
  struct supported_conversion_from_type<std::vector<T> > {         \
    typedef stl_seq_type_tag type;                                 \
  };                                                               \
  template <>                                                      \
  struct supported_conversion_from_type<std::deque<T> > {          \
    typedef stl_seq_type_tag type;                                 \
  };                                                               \
  template <>                                                      \
  struct supported_conversion_from_type<std::list<T> > {           \
    typedef stl_seq_type_tag type;                                 \
  };                                                               \
  template <>                                                      \
  struct supported_conversion_from_type<std::set<T> > {            \
    typedef stl_seq_type_tag type;                                 \
  };                                                               \
  template <>                                                      \
  struct supported_conversion_from_type<std::multiset<T> > {       \
    typedef stl_seq_type_tag type;                                 \
  };                                                               \
  template <>                                                      \
  struct supported_conversion_from_type<std::queue<T> > {          \
    typedef stl_seq_type_tag type;                                 \
  };                                                               \
  template <>                                                      \
  struct supported_conversion_from_type<std::stack<T> > {          \
    typedef stl_seq_type_tag type;                                 \
  };                                                               \
  template <>                                                      \
  struct supported_conversion_from_type<std::priority_queue<T> > { \
    typedef stl_seq_type_tag type;                                 \
  };

template <>
struct supported_conversion_to_type<ignore_token> {
  ...
  strtk_register_supported_iterator_type(sequence<char>::iterator)           \
      strtk_register_supported_iterator_type(sequence<char>::const_iterator) \
          strtk_register_supported_iterator_type(                            \
              sequence<unsigned char>::iterator)                             \
              strtk_register_supported_iterator_type(                        \
                  sequence<unsigned char>::const_iterator)

strtk_register_unsigned_type_tag(unsigned short) strtk_register_unsigned_type_tag(unsigned int) strtk_register_unsigned_type_tag(
    unsigned long) strtk_register_unsigned_type_tag(unsigned long long int)

    strtk_register_signed_type_tag(short) strtk_register_signed_type_tag(
        int) strtk_register_signed_type_tag(long) strtk_register_signed_type_tag(long long)

        strtk_register_real_type_tag(float) strtk_register_real_type_tag(
            double) strtk_register_real_type_tag(long double)

            strtk_register_byte_type_tag(unsigned char) strtk_register_byte_type_tag(
                signed char) strtk_register_byte_type_tag(char)

                strtk_register_hex_number_type_tag(hex_to_number_sink<short>) strtk_register_hex_number_type_tag(hex_to_number_sink<int>) strtk_register_hex_number_type_tag(hex_to_number_sink<long>) strtk_register_hex_number_type_tag(hex_to_number_sink<long long>) strtk_register_hex_number_type_tag(hex_to_number_sink<unsigned short>) strtk_register_hex_number_type_tag(
                    hex_to_number_sink<
                        unsigned int>) strtk_register_hex_number_type_tag(hex_to_number_sink<unsigned long>) strtk_register_hex_number_type_tag(hex_to_number_sink<unsigned long long int>)

                    strtk_register_base64_type_tag(
                        base64_to_number_sink<
                            short>) strtk_register_base64_type_tag(base64_to_number_sink<int>) strtk_register_base64_type_tag(base64_to_number_sink<long>) strtk_register_base64_type_tag(base64_to_number_sink<long long>) strtk_register_base64_type_tag(base64_to_number_sink<unsigned short>) strtk_register_base64_type_tag(base64_to_number_sink<unsigned int>) strtk_register_base64_type_tag(base64_to_number_sink<unsigned long>) strtk_register_base64_type_tag(base64_to_number_sink<unsigned long long int>)

                        strtk_register_stdstring_range_type_tag(std::string::iterator) strtk_register_stdstring_range_type_tag(
                            std::string::
                                const_iterator) strtk_register_stdstring_range_type_tag(char*)
                            strtk_register_stdstring_range_type_tag(signed char*) strtk_register_stdstring_range_type_tag(
                                unsigned char*) strtk_register_stdstring_range_type_tag(const char*)
                                strtk_register_stdstring_range_type_tag(
                                    const unsigned char*)

                                    strtk_register_supported_iterator_type(char*) strtk_register_supported_iterator_type(
                                        signed char*) strtk_register_supported_iterator_type(unsigned char*)
                                        strtk_register_supported_iterator_type(
                                            const char*) strtk_register_supported_iterator_type(const signed char*)
                                            strtk_register_supported_iterator_type(
                                                const unsigned char*)
                                                strtk_register_supported_iterator_type(
                                                    std::string::iterator)
                                                    strtk_register_supported_iterator_type(
                                                        ...
                                                        strtk_register_sequence_iterator_type(
                                                            ...
                                                            strtk_register_sequence_iterator_type(
                                                                ...
                                                                strtk_register_sink_type_tag(
                                                                    float) strtk_register_sink_type_tag(double)
                                                                    strtk_register_sink_type_tag(long double) strtk_register_sink_type_tag(signed char) strtk_register_sink_type_tag(char) strtk_register_sink_type_tag(short) strtk_register_sink_type_tag(int) strtk_register_sink_type_tag(long) strtk_register_sink_type_tag(long long) strtk_register_sink_type_tag(unsigned char) strtk_register_sink_type_tag(unsigned short) strtk_register_sink_type_tag(unsigned int) strtk_register_sink_type_tag(unsigned long) strtk_register_sink_type_tag(unsigned long long int) strtk_register_sink_type_tag(
                                                                        std::
                                                                            string)

                                                                        strtk_register_stl_container_to_string_conv_type_tag(
                                                                            float) strtk_register_stl_container_to_string_conv_type_tag(double)
                                                                            strtk_register_stl_container_to_string_conv_type_tag(long double) strtk_register_stl_container_to_string_conv_type_tag(signed char) strtk_register_stl_container_to_string_conv_type_tag(char) strtk_register_stl_container_to_string_conv_type_tag(short) strtk_register_stl_container_to_string_conv_type_tag(int) strtk_register_stl_container_to_string_conv_type_tag(long) strtk_register_stl_container_to_string_conv_type_tag(long long) strtk_register_stl_container_to_string_conv_type_tag(unsigned char) strtk_register_stl_container_to_string_conv_type_tag(unsigned short) strtk_register_stl_container_to_string_conv_type_tag(unsigned int) strtk_register_stl_container_to_string_conv_type_tag(
                                                                                unsigned long) strtk_register_stl_container_to_string_conv_type_tag(unsigned long long int) strtk_register_stl_container_to_string_conv_type_tag(std::
                                                                                                                                                                                                                                     string)

                                                                                strtk_register_inrange_type_tag(float) strtk_register_inrange_type_tag(double) strtk_register_inrange_type_tag(long double) strtk_register_inrange_type_tag(signed char) strtk_register_inrange_type_tag(char) strtk_register_inrange_type_tag(short) strtk_register_inrange_type_tag(
                                                                                    int) strtk_register_inrange_type_tag(long) strtk_register_inrange_type_tag(long long) strtk_register_inrange_type_tag(unsigned char) strtk_register_inrange_type_tag(unsigned short) strtk_register_inrange_type_tag(unsigned int)
                                                                                    strtk_register_inrange_type_tag(unsigned long) strtk_register_inrange_type_tag(
                                                                                        unsigned long long int) strtk_register_inrange_type_tag(std::string)

                                                                                        strtk_register_trim_type_tag(
                                                                                            float)
                                                                                            strtk_register_trim_type_tag(
                                                                                                double) strtk_register_trim_type_tag(long double) strtk_register_trim_type_tag(signed char) strtk_register_trim_type_tag(char) strtk_register_trim_type_tag(short) strtk_register_trim_type_tag(int) strtk_register_trim_type_tag(long) strtk_register_trim_type_tag(long long) strtk_register_trim_type_tag(unsigned char) strtk_register_trim_type_tag(unsigned short) strtk_register_trim_type_tag(unsigned int) strtk_register_trim_type_tag(unsigned long) strtk_register_trim_type_tag(unsigned long long int) strtk_register_trim_type_tag(std::
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      string)

                                                                                                strtk_register_trim_type_tag(
                                                                                                    truncated_int<
                                                                                                        short>)
                                                                                                    strtk_register_trim_type_tag(truncated_int<int>) strtk_register_trim_type_tag(truncated_int<long>) strtk_register_trim_type_tag(truncated_int<long long>) strtk_register_trim_type_tag(truncated_int<unsigned char>) strtk_register_trim_type_tag(truncated_int<unsigned short>) strtk_register_trim_type_tag(truncated_int<unsigned int>) strtk_register_trim_type_tag(
                                                                                                        truncated_int<
                                                                                                            unsigned long long int>)

                                                                                                        strtk_register_truncint_type_tag(
                                                                                                            short)
                                                                                                            strtk_register_truncint_type_tag(int) strtk_register_truncint_type_tag(long) strtk_register_truncint_type_tag(long long) strtk_register_truncint_type_tag(unsigned short) strtk_register_truncint_type_tag(
                                                                                                                unsigned int)
                                                                                                                strtk_register_truncint_type_tag(
                                                                                                                    unsigned long)
                                                                                                                    strtk_register_truncint_type_tag(
...
#undef strtk_register_iostream_precision
            ...
            inline bool string_to_type_converter_impl(Iterator& begin,
                                                      const Iterator end, T& t,
...
inline bool string_to_type_converter_impl(Iterator& itr_external,
  ...
  if (strtk_unlikely((length = std::distance(itr, end)) >
                     numeric<T>::bound_length))
    return false;

  strtk_disable_fallthrough_begin switch (length) {
#define case_stmt(N)                                 \
  case N:                                            \
    ...
    case_stmt(20) case_stmt(19) case_stmt(18) case_stmt(17) case_stmt(16)
        case_stmt(15) case_stmt(14) case_stmt(13) case_stmt(12) case_stmt(11)
            case_stmt(10) case_stmt(9) case_stmt(8) case_stmt(7) case_stmt(6)
                case_stmt(5) case_stmt(4) case_stmt(3) case_stmt(2) case 1
        : if (strtk_unlikely((digit = (*itr - '0')) >= 10)) return false;
#undef case_stmt
  }
  strtk_disable_fallthrough_end
    ...
    if (!((t < numeric<T>::m10) ||
          ((t == numeric<T>::m10) && (digit <= numeric<T>::ldpos))))
      return false;
  }
...
inline bool string_to_type_converter_impl(Iterator& itr_external,
                                          ...
                                          signed_type_tag) {
  Iterator itr;
  if ((itr = itr_external) == end) return false;
...
#define case_stmt(N)                                 \
  case N:                                            \
    ...
    case_stmt(19) case_stmt(18) case_stmt(17) case_stmt(16) case_stmt(15)
        case_stmt(14) case_stmt(13) case_stmt(12) case_stmt(11) case_stmt(10)
            case_stmt(9) case_stmt(8) case_stmt(7) case_stmt(6) case_stmt(5)
                case_stmt(4) case_stmt(3) case_stmt(2) case 1
        : if (strtk_unlikely((digit = (*itr - '0')) >= 10)) return false;
#undef case_stmt
  }
  strtk_disable_fallthrough_end
      ...
      if (length == numeric<T>::bound_length) {
    if (!((t < static_cast<T>(numeric<T>::m10)) ||
          ((t == numeric<T>::m10) && (static_cast<T>(digit) <= last_digit))))
      return false;
  }
...
#define strtk_process_digit                        \
  if (strtk_likely((digit = (*itr++ - '0')) < 10)) \
    ...
    break;                                         \
      ...
      case 4:
        strtk_process_digit case 3 : strtk_process_digit case 2
            : strtk_process_digit case 1
...
inline bool parse_nan(Iterator& itr, const Iterator end, T& t) {
  ...
  if (std::distance(itr, end) != static_cast<int>(nan_length)) return false;
...
// is advised that the following loop be: 4xPD1 and 1xPD2
      ...
      parse_digit_1(d) parse_digit_1(d)
#endif
  ...
  if (end != itr) {
    if ('.' == (*itr)) {
      ...
      while (end != itr) {
#ifdef strtk_enable_superscalar
        parse_digit_1(tmp_d) parse_digit_1(tmp_d) parse_digit_1(tmp_d)
#endif
...
#undef parse_digit_2
    ...
    if (end != itr) {
      ...
      if (('e' == c) || ('E' == c)) {
        ...
        if (!details::string_to_type_converter_impl_ref(
                ++itr, end, exp, details::signed_type_tag())) {
          if (end == itr)
            return false;
          else
            c = (*itr);
...
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
                                          T& t, byte_type_tag) {
  if (1 != std::distance(itr, end)) return false;
  t = static_cast<T>(*itr);
  itr = end;
  return true;
}
...
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
  ...
  if (!t.parse(itr, end)) return false;
  itr = end;
  return true;
}
...
inline bool type_to_string_converter_impl(T value, std::string& result,
  ...
  if (value) {
    while (value >= static_cast<T>(radix_sqr)) {
      ...
      memcpy(itr, &details::rev_3digit_lut[3 * (value - (temp_v * radix_cube))],
             3);
      value = temp_v;
    }
    ...
    while (value >= static_cast<T>(radix)) {
      ...
      memcpy(itr, &details::rev_2digit_lut[2 * (value - (temp_v * radix_sqr))],
             2);
      value = temp_v;
    }
...
define_tsci_type(short, unsigned short) define_tsci_type(int, unsigned int)
    define_tsci_type(long, unsigned long) define_tsci_type(long long,
                                                           unsigned long long)
#undef define_tsci_type
  ...
  if (value) {
    while (value >= radix_sqr) {
      ...
      value /= radix_cube;
      memcpy((itr -= 3),
             &details::rev_3digit_lut[3 * (v - (value * radix_cube))], 3);
    }
    ...
    while (value >= radix) {
      ...
      value /= radix_sqr;
      memcpy((itr -= 2),
             &details::rev_2digit_lut[2 * (v - (value * radix_sqr))], 2);
    }
...
inline bool type_to_string_converter_impl(const STLContainerType&, std::string&,
                                          stl_seq_type_tag) {
  // Generic conversion not supported for stl containers. Use joins or custom
  // converters.
  return false;
}
...
strtk_register_type_name(signed char) strtk_register_type_name(unsigned char)
    strtk_register_type_name(short) strtk_register_type_name(int)
        strtk_register_type_name(long) strtk_register_type_name(long long)
...
inline std::string type_name(const std::pair<T1, T2>& p) {
  static const std::string s = std::string("std::pair<" + type_name(p.first) +
                                           "," + type_name(p.second) + ">");
  return s;
}
...
inline std::size_t type_length(const std::string& s) { return s.size(); }
...
#define strtk_register_sequence_type_name(Type)                                \
  ...
  inline std::string type_name(const Type<T, Allocator>&) {                    \
    static const std::string s =                                               \
        std::string(#Type) + std::string("<" + details::type_name<T>() + ">"); \
    return s;                                                                  \
  }
...
class ext_string {
 public:
  ...
  explicit ext_string(const std::string& s) : s_(s) {}

  explicit ext_string(const char* s) : s_(s) {}

  explicit ext_string(const range::adapter<char>& r) : s_(r.begin(), r.end()) {}

  ext_string(const ext_string& es) : s_(es.s_) {}
  ...
  inline operator std::string() const { return s_; }

  inline std::string clone() const { return s_; }

  inline const std::string& as_string() const { return s_; }

  inline std::string& as_string() { return s_; }
  ...
  inline ext_string& remove_leading(const std::string& removal_set) {
    if (removal_set.empty())
      ...
      strtk::remove_leading(
          single_delimiter_predicate<std::string::value_type>(removal_set[0]),
          s_);
    else
  ...
  inline ext_string& remove_trailing(const std::string& removal_set) {
    if (removal_set.empty())
      ...
      strtk::remove_trailing(multiple_char_delimiter_predicate(removal_set),
                             s_);
    return (*this);
  }
  ...
  inline std::size_t split(const DelimiterPredicate& p, OutputIterator out,
                           const split_options::type split_option =
                               split_options::default_mode) const {
    return strtk::split(p, s_, out, split_option);
  }

  template <typename DelimiterPredicate, typename Allocator,
            template <typename, typename> class Sequence>
  ...
  template <typename DelimiterPredicate, typename OutputIterator>
  inline std::size_t split_n(const DelimiterPredicate& p, const std::size_t& n,
                             OutputIterator out,
                             const split_options::type split_option =
                                 split_options::default_mode) const {
...
inline bool write_file(const std::string& file_name,
  ...
  return write_file(file_name, const_cast<char*>(to_ptr(buffer)),
                    buffer.size());
}
...
inline bool files_identical(const std::string& file_name1,
  ...
  while (remaining_bytes >= block_size) {
    ...
    if (0 != std::memcmp(buffer1, buffer2, block_size)) return false;
    remaining_bytes -= block_size;
  }
  ...
  if (remaining_bytes > 0) {
    ...
    if (0 != std::memcmp(buffer1, buffer2, remaining_bytes)) return false;
    remaining_bytes = 0;
  }
...
inline bool read_at_offset(const std::string& file_name,
  ...
  return read_at_offset(stream, offset, const_cast<char*>(to_ptr(buffer)),
                        buffer_size);
}
...
template <typename T, typename Comparator, typename Allocator>
inline unsigned char* read_pod(
    unsigned char* data, const std::size_t& n,
    const std::multiset<T, Comparator, Allocator>& multiset) {
  T* ptr = reinterpret_cast<T>(data);
...
template <typename T, typename Comparator, typename Allocator>
inline unsigned char* write_pod(
    unsigned char* data,
    const std::multiset<T, Comparator, Allocator>& multiset) {
  T* ptr = reinterpret_cast<T>(data);
...
class prefix {
  ...
  struct node {
   public:
    ...
    inline void add_node(node_ptr n) { node_list_.push_back(n); }

    inline bool value_holder() const { return value_holder_; }

    inline const value_t& value() const { return value_; }

    inline const key_value_t& key() const { return key_value_; }
 ...
 public:
  ...
  inline bool find(const key_iterator_t begin, const key_iterator_t end,
                   value_t& v) const {
    if ((0 == head_) || (0 == std::distance(begin, end))) return false;
    ...
    while (end != itr) {
      ...
      if (0 == next_node) return false;

      parent = n;
      n = next_node;
      ++itr;
    }
    if (!parent->value_holder()) return false;
    v = parent->value();
    return true;
  }
  ...
  inline bool find_prefix(const key_iterator_t begin,
    ...
    while (end != itr) {
      if (0 == (n = n->get_node(*itr))) return false;
      ++itr;
    }
...
template <typename ValueType,
          typename KeyIterator = std::string::const_iterator>
struct prefix_trie {
...
class filter {
 ...
 public:
  ...
  filter(const filter& filter) { this->operator=(filter); }
  ...
  inline bool operator!=(const filter& f) const { return !operator==(f); }
  ...
  virtual ~filter() { delete[] bit_table_; }
  ...
  inline bool contains(const std::string& key) const {
    return contains(reinterpret_cast<const unsigned char*>(to_ptr(key)),
                    key.size());
  }
  ...
  inline bool write_to_file(const std::string& file_name) const {
    if (0 == table_size_) return false;
    const std::size_t buffer_size =
        sizeof(salt_count_) + sizeof(table_size_) + sizeof(raw_table_size_) +
        sizeof(projected_element_count_) + sizeof(inserted_element_count_) +
        sizeof(random_seed_) + sizeof(desired_false_positive_probability_) +
    ...
    bool result = writer(salt_count_) && writer(table_size_) &&
                  ...
                  writer(salt_) && writer(bit_table_, raw_table_size_);
    if (result) {
      writer(ostream);
    }
    ostream.close();
  ...
  inline bool read_from_file(const std::string& file_name) {
    ...
    if (0 != bit_table_) delete[] bit_table_;
    ...
    bool result = reader(salt_count_) && reader(table_size_) &&
                  ...
                  reader(salt_) && reader(bit_table_, raw_table_size_);
...
class compressible_filter : public filter {
 public:
  compressible_filter(const parameters& p) : filter(p) {
  ...
  inline bool compress(const double& percentage) {
    ...
    cell_type* tmp =
        new cell_type[static_cast<std::size_t>(new_table_size / bits_per_char)];
...
class attribute {
 public:
  ...
  inline bool operator==(const T& t) { return initialised_ && (t_ == t); }
  ...
  inline const T& operator()() const { return t_; }
  ...
  inline bool changed() const { return (initialised_ && (t_ != prev_t_)); }

  inline const T& value() const { return t_; }

  inline T& value() { return t_; }

  inline const T& previous() const { return prev_t_; }

  inline T& previous() { return prev_t_; }
...
class semantic_action_impl {
 private:
  ...
  class function_holder : public function_holder_base {
   public:
    explicit function_holder(Function& f) : function_(&f) {}
 ...
 public:
  semantic_action_impl() : function_holder_(0) {
  ...
  inline bool operator!() const { return (0 == function_holder_); }
  ...
  inline void assign(Function& f) {
    ...
    function_holder_ =
        construct<Function, type_size <= function_holder_buffer_size>::type(
            f, function_holder_buffer_);
  }

  inline semantic_action_impl& ref() { return (*this); }
...
#define strtk_register_attribute_type_tag(T)                          \
  template <>                                                         \
  struct supported_conversion_to_type<strtk::util::attribute<T> > {   \
    typedef attribute_type_tag type;                                  \
  };                                                                  \
  template <>                                                         \
  struct supported_conversion_from_type<strtk::util::attribute<T> > { \
    typedef attribute_type_tag type;                                  \
  };

strtk_register_attribute_type_tag(
    unsigned short) strtk_register_attribute_type_tag(unsigned int)
    strtk_register_attribute_type_tag(
        unsigned long) strtk_register_attribute_type_tag(unsigned long long int)
        strtk_register_attribute_type_tag(
            short) strtk_register_attribute_type_tag(int)
            strtk_register_attribute_type_tag(
                long) strtk_register_attribute_type_tag(long long)
                strtk_register_attribute_type_tag(
                    float) strtk_register_attribute_type_tag(double)
...
inline bool type_to_string_converter_impl(
    ...
    attribute_type_tag) {
  if (!attrib.initialised()) return false;
...
template <>
struct supported_conversion_to_type<strtk::util::semantic_action_impl> {
  typedef semantic_action_type_tag type;
};
template <>
struct supported_conversion_from_type<strtk::util::semantic_action_impl> {
  typedef semantic_action_type_tag type;
};
...
class value {
 private:
  ...
  class type_holder : public type_holder_base {
   public:
    ...
    explicit type_holder(T& t) : value_ptr_(&t) {}
    ...
    inline operator T() const { return (*value_ptr_); }
 ...
 public:
  ...
  inline bool operator!() const { return (0 == type_holder_); }
 ...
 private:
  ...
  enum {
    type_holder_buffer_size = 2 * sizeof(type_holder<unsigned long long int>)
  };
...
template <typename Key, typename T, typename Comparator, typename MapAllocator,
          typename OutputIterator>
inline void make_key_list_impl(
    const std::map<Key, T, Comparator, MapAllocator>& map, OutputIterator out) {
  if (map.empty()) return;
...
template <typename Key, typename T, typename Comparator, typename MapAllocator,
          typename SetAllocator>
inline void make_key_list(
    const std::map<Key, T, Comparator, MapAllocator>& map,
...
#define strtk_def_mkky_list(Type)                                   \
  template <typename Key, typename T, typename Comparator,          \
            typename MapAllocator, typename SequenceAllocator>      \
  inline void make_key_list(                                        \
      const std::map<Key, T, Comparator, MapAllocator>& map,        \
...
strtk_def_mkky_list(std::vector) strtk_def_mkky_list(std::deque)
    strtk_def_mkky_list(std::list) strtk_def_mkky_list(std::queue)
#undef strtk_def_mkky_list

        template <typename Key, typename T, typename Comparator,
                  typename MapAllocator, typename OutputIterator>
...
template <typename Key, typename T, typename Comparator, typename MapAllocator,
          ...
          template <typename, typename> class Sequence>
inline void make_value_list(
    const std::multimap<Key, T, Comparator, MapAllocator>& map, const Key& key,
...
template <typename Predicate, typename T, typename Comparator,
          typename Allocator>
inline void delete_if(const Predicate& p,
...
strtk_def_clear_seq(std::vector) strtk_def_clear_seq(std::deque)
    strtk_def_clear_seq(std::list) strtk_def_clear_seq(std::priority_queue)
        strtk_def_clear_seq(std::stack) strtk_def_clear_seq(std::queue)
...
#define strtk_def_clear_map(Type)                                \
  ...
  inline void clear(Type<T, Comparator, Allocator>& cont) {      \
    ...
    std::swap(cont, null_cont);                                  \
            ...
            strtk_def_clear_map(std::set) strtk_def_clear_map(std::multiset)
...
class column_selector_base {
 public:
  ...
  inline csb_t& operator*() { return (*this); }

  inline csb_t& operator++() { return (*this); }

  inline csb_t operator++(int) { return (*this); }
 ...
 protected:
  class colsel_value_list {
   public:
    ...
    colsel_value_list() : current_index(0) {
  ...
  inline colsel_value_list& cvl() { return cvl_; }
...
class column_selector_impl : public column_selector_base<
                                 column_selector_impl<T0, T1, T2, T3, T4, T5,
                                                      T6, T7, T8, T9, T10, T11>,
                                 12> {
 public:
  ...
  column_selector_impl(const column_list_t& column_list, T0& t0, T1& t1, T2& t2,
...
class column_selector_impl<T0, T1, T2, T3, T4, T5, T6>
 ...
 public:
  typedef column_selector_base<column_selector_impl<T0, T1, T2, T3, T4, T5, T6>,
                               7>
      csb_t;
...
class column_selector_impl<T0, T1, T2, T3, T4, T5>
    : public column_selector_base<column_selector_impl<T0, T1, T2, T3, T4, T5>,
                                  6> {
 public:
...
class column_selector_impl<T0, T1, T2, T3, T4>
 ...
 public:
  typedef column_selector_base<column_selector_impl<T0, T1, T2, T3, T4>, 5>
      csb_t;
...
inline details::column_list_impl<12> column_list(
    const std::size_t& idx0, const std::size_t& idx1, const std::size_t& idx2,
    const std::size_t& idx3, const std::size_t& idx4, const std::size_t& idx5,
    const std::size_t& idx6, const std::size_t& idx7, const std::size_t& idx8,
...
inline details::column_list_impl<11> column_list(
    const std::size_t& idx0, const std::size_t& idx1, const std::size_t& idx2,
    const std::size_t& idx3, const std::size_t& idx4, const std::size_t& idx5,
    const std::size_t& idx6, const std::size_t& idx7, const std::size_t& idx8,
    const std::size_t& idx9, const std::size_t& idx10) {
...
inline details::column_list_impl<10> column_list(
    const std::size_t& idx0, const std::size_t& idx1, const std::size_t& idx2,
    const std::size_t& idx3, const std::size_t& idx4, const std::size_t& idx5,
    const std::size_t& idx6, const std::size_t& idx7, const std::size_t& idx8,
...
inline details::column_list_impl<9> column_list(
    const std::size_t& idx0, const std::size_t& idx1, const std::size_t& idx2,
    const std::size_t& idx3, const std::size_t& idx4, const std::size_t& idx5,
    const std::size_t& idx6, const std::size_t& idx7, const std::size_t& idx8) {
...
inline details::column_list_impl<8> column_list(
    const std::size_t& idx0, const std::size_t& idx1, const std::size_t& idx2,
    const std::size_t& idx3, const std::size_t& idx4, const std::size_t& idx5,
...
inline details::column_list_impl<7> column_list(
    const std::size_t& idx0, const std::size_t& idx1, const std::size_t& idx2,
    const std::size_t& idx3, const std::size_t& idx4, const std::size_t& idx5,
...
inline details::column_list_impl<6> column_list(
    const std::size_t& idx0, const std::size_t& idx1, const std::size_t& idx2,
    const std::size_t& idx3, const std::size_t& idx4, const std::size_t& idx5) {
...
column_selector(const details::column_list_impl<11>& col_list, T0& t0, T1& t1,
                ...
                T10& t10) {
  return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9,
                                       T10>(col_list, t0, t1, t2, t3, t4, t5,
                                            t6, t7, t8, t9, t10);
}
...
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7,
                                              T8, T9>
column_selector(const details::column_list_impl<10>& col_list, T0& t0, T1& t1,
...
template <typename T0, typename T1, typename T2, typename T3, typename T4,
          typename T5, typename T6, typename T7, typename T8>
inline
    typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8>
    column_selector(const details::column_list_impl<9>& col_list, T0& t0,
                    T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7,
                    T8& t8) {
...
template <typename T0, typename T1, typename T2, typename T3, typename T4,
          typename T5, typename T6, typename T7>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7>
column_selector(const details::column_list_impl<8>& col_list, T0& t0, T1& t1,
...
column_selector(const details::column_list_impl<7>& col_list, T0& t0, T1& t1,
                T2& t2, T3& t3, T4& t4, T5& t5, T6& t6) {
  return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6>(
...
column_selector(const details::column_list_impl<6>& col_list, T0& t0, T1& t1,
                T2& t2, T3& t3, T4& t4, T5& t5) {
  return details::column_selector_impl<T0, T1, T2, T3, T4, T5>(col_list, t0, t1,
                                                               t2, t3, t4, t5);
}

template <typename T0, typename T1, typename T2, typename T3, typename T4>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4>
column_selector(const details::column_list_impl<5>& col_list, T0& t0, T1& t1,
                T2& t2, T3& t3, T4& t4) {
  return details::column_selector_impl<T0, T1, T2, T3, T4>(col_list, t0, t1, t2,
                                                           t3, t4);
}
...
struct compose_st_selector_impl<T, 9> {
  ...
  static inline type create(const column_list_t& col_list,
    ...
    typename std::list<T, Allocator>::iterator b = list.begin();
    return type(col_list, *(b), *inc(b, 1), *inc(b, 2), *inc(b, 3), *inc(b, 4),
                *inc(b, 5), *inc(b, 6), *inc(b, 7), *inc(b, 8));
  }
};
...
struct compose_st_selector_impl<T, 10> {
  ...
  static inline type create(const column_list_t& col_list,
    ...
    typename std::list<T, Allocator>::iterator b = list.begin();
    return type(col_list, *(b), *inc(b, 1), *inc(b, 2), *inc(b, 3), *inc(b, 4),
                *inc(b, 5), *inc(b, 6), *inc(b, 7), *inc(b, 8), *inc(b, 9));
  }
};

}  // namespace details

template <std::size_t N, typename T, typename Allocator,
          template <typename, typename> class Sequence>
inline typename details::compose_st_selector_impl<T, N>::type column_selector(
    const details::column_list_impl<N>& col_list, Sequence<T, Allocator>& seq) {
  if (seq.size() >= N) {
...
class column_selector_iterator_impl {
 public:
  ...
  inline csii_t& operator*() { return (*this); }

  inline csii_t& operator++() { return (*this); }

  inline csii_t operator++(int) { return (*this); }
...
#define strtk_parse_col_token(Index)                                 \
  if (!string_to_type_converter(token_list[Index].first,             \
                                token_list[Index].second, t##Index)) \
    return false;

#define strtk_parse_col_token_seq(Index)                               \
  if (!string_to_type_converter(token_list[Index].first,               \
                                token_list[Index].second, seq[Index])) \
    return false;
...
#define strtk_parse_columns_impl(NN)                                         \
  ...
  const std::size_t parsed_token_count = split_n<InputIterator, csii_t&>(    \
      delimiters, begin, end, token_count, csii,                             \
      split_options::compress_delimiters);                                   \
  if (token_count > parsed_token_count) return false;

#define strk_parse_col_seq                                                   \
  return parse_columns(to_ptr(data), to_ptr(data) + data.size(), delimiters, \
                       column_list, seq);
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
  ...
  strtk_parse_columns_impl(12) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
          strtk_parse_col_token(5) strtk_parse_col_token(6)
              strtk_parse_col_token(7) strtk_parse_col_token(8)
                  strtk_parse_col_token(9) strtk_parse_col_token(10)
                      strtk_parse_col_token(11) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
  ...
  strtk_parse_columns_impl(11) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
          strtk_parse_col_token(5) strtk_parse_col_token(6)
              strtk_parse_col_token(7) strtk_parse_col_token(8)
                  strtk_parse_col_token(9)
                      strtk_parse_col_token(10) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
  ...
  strtk_parse_columns_impl(10) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
          strtk_parse_col_token(5) strtk_parse_col_token(6)
              strtk_parse_col_token(7) strtk_parse_col_token(8)
                  strtk_parse_col_token(9) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6, T7& t7, T8& t8) {
  strtk_parse_columns_impl(9) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
          strtk_parse_col_token(5) strtk_parse_col_token(6)
              strtk_parse_col_token(7) strtk_parse_col_token(8) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6, T7& t7) {
  strtk_parse_columns_impl(8) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
          strtk_parse_col_token(5) strtk_parse_col_token(6)
              strtk_parse_col_token(7) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6) {
  strtk_parse_columns_impl(7) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
          strtk_parse_col_token(5) strtk_parse_col_token(6) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5) {
  strtk_parse_columns_impl(6) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
          strtk_parse_col_token(5) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
  ...
  strtk_parse_columns_impl(5) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3)
          strtk_parse_col_token(4) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
  ...
  strtk_parse_columns_impl(4) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) strtk_parse_col_token(3) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          T0& t0, T1& t1, T2& t2) {
  strtk_parse_columns_impl(3) strtk_parse_col_token(0) strtk_parse_col_token(1)
      strtk_parse_col_token(2) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          T0& t0, T1& t1) {
  strtk_parse_columns_impl(2) strtk_parse_col_token(0)
      strtk_parse_col_token(1) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          T0& t0) {
  strtk_parse_columns_impl(1) strtk_parse_col_token(0) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<12>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(12) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
              strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6)
                  strtk_parse_col_token_seq(7) strtk_parse_col_token_seq(8)
                      strtk_parse_col_token_seq(9) strtk_parse_col_token_seq(10)
                          strtk_parse_col_token_seq(11) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<11>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(11) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
              strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6)
                  strtk_parse_col_token_seq(7) strtk_parse_col_token_seq(8)
                      strtk_parse_col_token_seq(9)
                          strtk_parse_col_token_seq(10) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<10>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(10) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
              strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6)
                  strtk_parse_col_token_seq(7) strtk_parse_col_token_seq(8)
                      strtk_parse_col_token_seq(9) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<9>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(9) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
              strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6)
                  strtk_parse_col_token_seq(7)
                      strtk_parse_col_token_seq(8) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<8>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(8) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
              strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6)
                  strtk_parse_col_token_seq(7) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<7>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(7) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
              strtk_parse_col_token_seq(5)
                  strtk_parse_col_token_seq(6) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<6>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(6) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
              strtk_parse_col_token_seq(5) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<5>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(5) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<4>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(4) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2)
          strtk_parse_col_token_seq(3) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<3>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(3) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) strtk_parse_col_token_seq(2) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<2>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(2) strtk_parse_col_token_seq(0)
      strtk_parse_col_token_seq(1) return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
                          ...
                          const details::column_list_impl<1>& column_list,
                          Sequence<T, Allocator>& seq) {
  strtk_parse_columns_impl(1) strtk_parse_col_token_seq(0) return true;
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6, T7& t7, T8& t8, T9& t9, T10& t10, T11& t11) {
  strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10,
      t11 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6, T7& t7, T8& t8, T9& t9, T10& t10) {
  strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7, t8, t9,
      t10 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6, T7& t7, T8& t8, T9& t9) {
  strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7, t8,
      t9 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6, T7& t7, T8& t8) {
  strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7,
      t8 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6, T7& t7) {
  strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5,
                          T6& t6) {
  strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          const details::column_list_impl<6>& column_list,
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5) {
  strtk_parse_col_begin() t0, t1, t2, t3, t4, t5 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          const details::column_list_impl<5>& column_list,
                          T0& t0, T1& t1, T2& t2, T3& t3, T4& t4) {
  strtk_parse_col_begin() t0, t1, t2, t3, t4 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          const details::column_list_impl<4>& column_list,
                          T0& t0, T1& t1, T2& t2, T3& t3) {
  strtk_parse_col_begin() t0, t1, t2, t3 strtk_parse_col_end();
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          const details::column_list_impl<3>& column_list,
                          T0& t0, T1& t1, T2& t2) {
  strtk_parse_col_begin() t0, t1, t2 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          const details::column_list_impl<2>& column_list,
                          T0& t0, T1& t1) {
  strtk_parse_col_begin() t0, t1 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          const details::column_list_impl<1>& column_list,
                          T& t) {
  strtk_parse_col_begin() t strtk_parse_col_end()
}
...
struct size_impl {
  static inline bool cmp(ptr, ptr) { return true; }
};
...
template <>
struct memcmp_n_impl<0> {
  static inline bool process(ptr, ptr) { return true; }
};
}  // namespace details
...
inline bool set_array(unsigned char (&a)[N], const std::string& s,
  ...
  if ((s.size() < N) && pad) std::fill_n(&a[s.size()], N - s.size(), padding);
  return true;
}
...
inline bool set_array(unsigned char (&dest)[N], unsigned char (&src)[M],
  ...
  if ((M < N) && pad) std::fill_n(&dest[M], N - M, padding);
  return true;
}
...
inline void reverse(std::string& s) { std::reverse(s.begin(), s.end()); }

inline void fill(std::string& s, const std::string::value_type v) {
  std::fill(const_cast<char*>(to_ptr(s)),
            const_cast<char*>(to_ptr(s) + s.size()), v);
}
...
struct options {
  ...
  options() : pair_block_delimiter(0), pair_delimiter(0) {}
...
class parser {
 public:
    ...
    if (!ignore_failures) {
      ...
      for (std::size_t i = 0; i < pair_count; ++i) {
        ...
        if (0 == std::distance(r.first, r.second))
          continue;
        else if (!split_pair(r.first, r.second, pair_delimiter_sdp_, key_range,
                             value_range))
          return false;
  ...
  inline std::size_t failure_count() const { return parse_failures_; }

 private:
  class pair_token_processor {
   public:
    pair_token_processor(parser<KeyValueMap>& p) : parser_(p) {}

    inline void operator()(const range_type& r) {
      if (r.first == r.second) return;

      if (split_pair(r.first, r.second, parser_.pair_delimiter_sdp_, key_range,
                     value_range)) {
        if (parser_.kv_map_(key_range, value_range)) return;
      }
...
class uintkey_map {
 ...
 public:
  ...
  struct options : public general_options {
    options() : general_options(), key_count(0) {}
  ...
  inline bool operator()(const Range& key_range, const Range& value_range) {
    ...
    if (!fast::numeric_convert(distance(key_range), key_range.first, key, true))
      return false;
    if (key >= value_lut_.size()) return false;
...
struct keygen {
  static inline KType transform(const Range&) { return KType(); }
};
...
struct keygen<Range, unsigned int> {
  static inline unsigned int transform(const Range& key_range) {
    ...
    if (strtk::fast::numeric_convert(
            std::distance(key_range.first, key_range.second), key_range.first,
            result, true))
      return result;
    else
...
class key_map {
 public:
  ...
  template <typename Range>
  inline bool operator()(const Range& key_range, const Range& value_range) {
    if (!key_validator_(key_range)) return false;
    if (!val_validator_(value_range)) return false;
    typename map_type::iterator itr =
        value_map_.find(details::keygen<Range, key_type>::transform(key_range));
    if (value_map_.end() == itr) return false;
...
static inline std::ostream& operator<<(
    std::ostream& os,
    const strtk::std_string::tokenizer<
        strtk::single_delimiter_predicate<unsigned char> >::type::iterator&
        range) {
  os << std::string((*range).first, (*range).second);
  return os;
}
...
#define strtk_register_pair_to_ostream(Iterator)                      \
  static inline std::ostream& operator<<(                             \
      std::ostream& os, const std::pair<Iterator, Iterator>& range) { \
    os << std::string(range.first, range.second);                     \
    return os;                                                        \
  }                                                                   \
  static inline std::ostream& operator<<(                             \
      std::ostream& os, std::pair<Iterator, Iterator>& range) {       \
    os << std::string(range.first, range.second);                     \
    return os;                                                        \
  }

strtk_register_pair_to_ostream(char*) strtk_register_pair_to_ostream(
    unsigned char*) strtk_register_pair_to_ostream(const char*)
...
class timer {
 public:
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32)
  timer() : in_use_(false) { QueryPerformanceFrequency(&clock_frequency_); }
  ...
  inline double time() const { return usec_time() * 0.000001; }
...
class scoped_timer {
 public:
  scoped_timer(double& time_value) : time_value_(time_value) { t_.start(); }
