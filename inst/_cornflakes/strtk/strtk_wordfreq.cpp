...
struct line_parser {
 public:
  line_parser(unsigned long long& word_count, map_t& map, Predicate& p)
      : word_count_(word_count), map_(map), p_(p) {
    str_.reserve(32);
  }
...
int main(int argc, char* argv[]) {
  ...
  while (word_list.end() != itr) {
    printf("%s %10d %10.9f\n",
           strtk::text::right_align(15, ' ', itr->first).c_str(), itr->second,
