...
struct parse_line {
 public:
  parse_line(Container& c, Predicate& p) : c_(&c), p_(&p) {}

  inline void operator()(const std::string& s) {
    strtk::split(*p_, s, strtk::range_to_type_back_inserter(*c_),
...
void parse_text(const std::string& file_name, Container& c) {
  ...
  parse_line<Container, strtk::multiple_char_delimiter_predicate> pl(c,
                                                                     predicate);
