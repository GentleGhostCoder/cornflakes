...
const std::string not_of_interest_list[] = {
    "as", "at",       "but", "by",   "for", "in", "like", "next", "of",
    "on", "opposite", "out", "past", "to",  "up", "via",  ""};

const std::size_t not_of_interest_list_size =
    sizeof(not_of_interest_list) / sizeof(std::string);

template <typename Container, typename Predicate>
struct parse_line {
 public:
  typedef typename strtk::filter_on_match<
      strtk::range_to_type_back_inserter_iterator<Container> >
      filter_type;
  ...
  parse_line(Container& c, Predicate& p)
      : c_(c),
        ...
        filter_(reinterpret_cast<const std::string*>(not_of_interest_list),
                ...
                strtk::range_to_type_back_inserter(c_), true, false) {}
...
void parse_text(const std::string& file_name, Container& c) {
  ...
  typedef parse_line<Container, strtk::multiple_char_delimiter_predicate>
      pl_type;
