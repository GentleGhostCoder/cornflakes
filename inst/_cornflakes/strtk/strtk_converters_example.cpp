...
void example03() {
  ...
  for (std::size_t k = 1; k < size; ++k) {
    ...
    std::size_t b64_size =
        strtk::convert_bin_to_base64(buffer1, buffer1 + k, b64_buffer);
    ...
    for (std::size_t i = 0; i < k; ++i) {
      if (buffer1[i] != buffer2[i]) {
        std::cout
            << "example03 - failed bin-b64 conversions, error at position: "
            << i << std::endl;
        return;
      }
    }
...
void bin2hex_speed_test() {
...

  {
    ...
    printf("[bin2hex] Data Size: %6lluMB  Total time:%8.4f  Rate: %6.2fMB/s\n",
           static_cast<unsigned long long>(rounds * r1.size()) /
               strtk::one_megabyte,
           t.time(), (rounds * r1.size()) / (1048576.0 * t.time()));
  }
