...
class ipv4_parser {
  typedef strtk::tokenizer<const char*,
                           strtk::single_delimiter_predicate<char> >
      tokenizer_type;

 public:
  ipv4_parser() : predicate_('.') {}
  ...
  inline bool operator()(const std::string& data, unsigned char octet[4]) {
    ...
    if (4 != strtk::split_n(predicate_, data, 4, token)) return false;
 ...
 private:
  inline bool process_token(const std::pair<const char*, const char*>& token,
                            unsigned char& octet) {
    unsigned int v = 0;
    if (!strtk::fast::numeric_convert(strtk::distance(token), token.first, v,
                                      true))
      return false;
...
void print_octet(const unsigned char octet[4]) {
  strtk::ext_string s;
  s << (unsigned int)(octet[0]) << "." << (unsigned int)(octet[1]) << "."
    << (unsigned int)(octet[2]) << "." << (unsigned int)(octet[3]);
  std::cout << "IP (octet) = "
            << strtk::text::right_align(15, ' ', s.as_string());
}

int main() {
  unsigned char octet[4] = {0};

  static const std::string ip[] = {"1.1.1.1", "12.12.12.12", "123.123.123.123"};
  ...
  for (std::size_t i = 0; i < ip_size; ++i) {
    ...
    if (!parser(ip[i], ip_int))
      ...
      std::cout << "\tIP(int): " << strtk::text::right_align(' ', ip_int)
                << std::endl;
  }
