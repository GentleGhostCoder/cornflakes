...
int main() {
  ...
  static const std::size_t element_count =
      2 * static_cast<std::size_t>(strtk::n_choose_k(n, k));
...

  {
    ...
    printf(
        "[insert ] Element Count: %llu\tTotal Time: %5.3fsec\tRate: "
        "%10.3felem/sec\n",
        static_cast<unsigned long long>(filter.element_count()), timer.time(),
...

  {
    ...
    while (buf_end != buf_itr) {
      if (!filter.contains(buf_itr, k)) {
        std::cout << "Error: Failed to find: "
                  << std::string(buf_itr, buf_itr + k) << std::endl;
      }
    ...
    printf(
        "[contain] Element Count: %llu\tTotal Time: %5.3fsec\tRate: "
        "%10.3felem/sec\n",
        static_cast<unsigned long long>(filter.element_count()), timer.time(),
        element_count / timer.time());
  }

  {
    ...
    printf(
        "[FPC    ] Element Count: %llu\tFalse Positive Count: %d\tFalse "
        "Positive Probability: %9.8f\tTotal Time: %5.3fsec\tRate: "
        "%10.3felem/sec\n",
        static_cast<unsigned long long>(small_element_count),
        false_positive_count,
        (1.0 * false_positive_count) / small_element_count, timer.time(),
    ...
    if (!filter.write_to_file("bloom_filter.bin")) {
      ...
      while (buf_end != buf_itr) {
        if (!secondary_filter.contains(buf_itr, k)) {
          std::cout << "Error: Failed to find: "
                    << std::string(buf_itr, buf_itr + k)
                    << " in secondary filter!" << std::endl;
          ++failures;
        }
