...
/// restart
void SHA3::reset() {
  for (size_t i = 0; i < StateSize; i++) m_hash[i] = 0;
...
void SHA3::processBuffer() {
  ...
  // fill with zeros
  while (offset < m_blockSize) m_buffer[offset++] = 0;
...
std::string SHA3::getHash() {
  ...
  for (unsigned int i = 0; i < StateSize; i++) oldHash[i] = m_hash[i];
  ...
  // restore state
  for (unsigned int i = 0; i < StateSize; i++) m_hash[i] = oldHash[i];
